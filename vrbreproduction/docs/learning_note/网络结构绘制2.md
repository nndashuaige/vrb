...
| 章节 | 内容 | 主要查询内容 |
| --- | --- | --- |
| 第 1 节 | 默认配置与符号 | 默认开关值、`B/T/N/H/W/E/M/L` 等符号 |
| 第 2 节 | VRB 总体网络结构 | `N01-N48` 网络节点、输入输出 shape、两条主线 |
| 第 3 节 | Ground Truth、Loss 与总体网络节点对照 | 论文数据处理节点 `Dxx` 如何对接网络节点 `Nxx` |
| 第 4 节 | 数据处理节点变量详解 | `Dxx` 节点中每个变量的含义、来源、去向和 shape |
| 第 5 节 | Loss 中网络变量详解 | `mu/recon_x/future_hands/obj_loss/traj_loss` 等训练变量 |
| 第 6 节 | ResNet50 分支 shape 对照 | `resnet18` 与 `resnet50` 的通道差异 |
| 第 7 节 | 读源码时最容易混淆的点 | 参数名、heatmap 与坐标监督、VAE 随机性等易错点 |
| 第 8 节 | 节点详解 | `N40-N47` 等关键网络节点的展开解释 |

## 2. VRB 总体网络结构

```mermaid
flowchart TD
    scene["N01 模块: 原始场景图像<br/>输入: 无<br/>输入shape: -<br/>输出: image_pil<br/>输出shape: H0 x W0 x 3<br/>参考: 第 2 节"]
    langsam["N02 模块: LangSAM 目标分割/检测<br/>输入: image_pil + objects<br/>输入shape: H0 x W0 x 3 + 目标词列表<br/>输出: boxes / masks<br/>输出shape: boxes约 K x 4<br/>参考: 第 2 节"]
    boxes["N03 模块: bbox 选择与整理<br/>输入: boxes / masks<br/>输入shape: boxes约 K x 4<br/>输出: 每个目标的 crop box<br/>输出shape: [y1, x1, y2, x2]<br/>参考: 第 2 节"]
    crop["N04 模块: crop + Resize + Normalize<br/>输入: image_pil + crop box<br/>输入shape: H0 x W0 x 3 + [4]<br/>输出: inp_img<br/>输出shape: 1 x 3 x 224 x 224<br/>参考: 第 2 节"]
    img["N05 模块: VRB 输入图像张量<br/>输入: inp_img<br/>输入shape: 1 x 3 x 224 x 224<br/>输出: img / feat参数<br/>输出shape: B x 3 x 224 x 224<br/>参考: 第 2 节"]

    subgraph vrb["VRBModel.forward / inference"]
        encimg["N06 模块: encode_img(img)<br/>输入: img<br/>输入shape: B x 3 x 224 x 224<br/>输出: fe, mu, cvar<br/>输出shape: fe B x src_in_features，mu B x 5 x 2，cvar 0或B x 5 x 2<br/>参考: 第 2 节"]
        resq{"N07 模块: resnet_type 开关<br/>输入: args.resnet_type<br/>输入shape: 标量字符串<br/>输出: resnet18 或 resnet50 分支<br/>输出shape: -<br/>参考: 第 1 节, 第 6 节"}
        r18note["N08 模块: resnet18 配置<br/>输入: resnet_type=resnet18<br/>输入shape: 标量<br/>输出: src_in_features / conv4通道<br/>输出shape: 512 / B x 512 x 7 x 7<br/>参考: 第 1 节, 第 6 节"]
        r50note["N09 模块: resnet50 配置<br/>输入: resnet_type=resnet50<br/>输入shape: 标量<br/>输出: src_in_features / conv4通道<br/>输出shape: 2048 / B x 2048 x 7 x 7<br/>参考: 第 6 节"]

        subgraph backbone["共享 ResNet / R3M-like backbone（见第 2 节与第 6 节）"]
            stem["N10 模块: ResNet stem<br/>输入: img<br/>输入shape: B x 3 x 224 x 224<br/>输出: stem特征 x<br/>输出shape: B x 64 x 56 x 56<br/>参考: 第 2 节"]
            l1["N11 模块: ResNet layer1<br/>输入: stem特征 x<br/>输入shape: B x 64 x 56 x 56<br/>输出: conv1变量<br/>输出shape: B x 64 x 56 x 56<br/>参考: 第 2 节"]
            l2["N12 模块: ResNet layer2<br/>输入: conv1变量<br/>输入shape: B x 64 x 56 x 56<br/>输出: conv2变量<br/>输出shape: res18 B x 128 x 28 x 28，res50 B x 512 x 28 x 28<br/>参考: 第 2 节, 第 6 节"]
            l3["N13 模块: ResNet layer3<br/>输入: conv2变量<br/>输入shape: res18 B x 128 x 28 x 28，res50 B x 512 x 28 x 28<br/>输出: conv3变量<br/>输出shape: res18 B x 256 x 14 x 14，res50 B x 1024 x 14 x 14<br/>参考: 第 2 节, 第 6 节"]
            l4["N14 模块: ResNet layer4<br/>输入: conv3变量<br/>输入shape: res18 B x 256 x 14 x 14，res50 B x 1024 x 14 x 14<br/>输出: conv4变量<br/>输出shape: res18 B x 512 x 7 x 7，res50 B x 2048 x 7 x 7<br/>参考: 第 2 节, 第 6 节"]
            pool["N15 模块: avgpool + flatten + fc Identity<br/>输入: conv4变量<br/>输入shape: res18 B x 512 x 7 x 7，res50 B x 2048 x 7 x 7<br/>输出: 全局语义特征 feat<br/>输出shape: res18 B x 512，res50 B x 2048<br/>参考: 第 2 节, 第 6 节"]
            fe["N16 模块: fe 全局特征节点<br/>输入: feat<br/>输入shape: res18 B x 512，res50 B x 2048<br/>输出: fe<br/>输出shape: res18 B x 512，res50 B x 2048<br/>参考: 第 2 节, 第 6 节"]
        end

        subgraph contact["接触点分支 Contact Head（见第 2 节）"]
            d1["N17 模块: deconv1 + BN + ReLU<br/>输入: conv4变量<br/>输入shape: res18 B x 512 x 7 x 7，res50 B x 2048 x 7 x 7<br/>输出: 上采样特征 h<br/>输出shape: res18 B x 256 x 14 x 14，res50 B x 1024 x 14 x 14<br/>参考: 第 2 节, 第 6 节"]
            add3["N18 模块: skip add h + conv3<br/>输入: h + conv3变量<br/>输入shape: res18 B x 256 x 14 x 14，res50 B x 1024 x 14 x 14<br/>输出: 融合特征<br/>输出shape: res18 B x 256 x 14 x 14，res50 B x 1024 x 14 x 14<br/>参考: 第 2 节, 第 6 节"]
            d2["N19 模块: deconv2 + BN + ReLU<br/>输入: 融合特征<br/>输入shape: res18 B x 256 x 14 x 14，res50 B x 1024 x 14 x 14<br/>输出: 上采样特征 h<br/>输出shape: res18 B x 128 x 28 x 28，res50 B x 512 x 28 x 28<br/>参考: 第 2 节, 第 6 节"]
            add2["N20 模块: skip add h + conv2<br/>输入: h + conv2变量<br/>输入shape: res18 B x 128 x 28 x 28，res50 B x 512 x 28 x 28<br/>输出: 融合特征<br/>输出shape: res18 B x 128 x 28 x 28，res50 B x 512 x 28 x 28<br/>参考: 第 2 节, 第 6 节"]
            d3["N21 模块: deconv3 + BN + ReLU<br/>输入: 融合特征<br/>输入shape: res18 B x 128 x 28 x 28，res50 B x 512 x 28 x 28<br/>输出: decoder h<br/>输出shape: res18 B x 64 x 56 x 56，res50 B x 128 x 110 x 110<br/>参考: 第 2 节, 第 6 节"]
            attnq{"N22 模块: attn_kp 开关<br/>输入: decoder h + attn_kp<br/>输入shape: h为 B x C x H x W，attn_kp为布尔值<br/>输出: attention路径或heatmap路径<br/>输出shape: -<br/>参考: 第 2 节"}
            cb_attn["N23 模块: attn=True 的 cb1 + cb2<br/>输入: decoder h<br/>输入shape: B x C x H x W<br/>输出: 接触点空间特征 x<br/>输出shape: B x 64 x H x W<br/>参考: 第 2 节"]
            flat["N24 模块: flatten + transpose<br/>输入: 接触点空间特征 x<br/>输入shape: B x 64 x H x W<br/>输出: 空间token序列<br/>输出shape: B x H*W x 64<br/>参考: 第 2 节"]
            pos2d["N25 模块: 2D位置编码相加<br/>输入: 空间token序列<br/>输入shape: B x H*W x 64<br/>输出: 带位置的空间token<br/>输出shape: B x H*W x 64<br/>参考: 第 2 节"]
            sattn["N26 模块: MultiheadAttention<br/>输入: Q/K/V空间token<br/>输入shape: B x H*W x 64<br/>输出: attention token<br/>输出shape: B x H*W x 64<br/>参考: 第 2 节"]
            restore["N27 模块: transpose + view<br/>输入: attention token<br/>输入shape: B x H*W x 64<br/>输出: attn_output<br/>输出shape: B x 64 x H x W<br/>参考: 第 2 节"]
            fcq{"N28 模块: attn_kp_fc 开关<br/>输入: attn_output + attn_kp_fc<br/>输入shape: B x 64 x H x W + 布尔值<br/>输出: A直接回归路径或B heatmap路径<br/>输出shape: -<br/>参考: 第 2 节"}
            fcmu["N29 模块: adaptive avg pool + fc_mu<br/>输入: attn_output<br/>输入shape: B x 64 x H x W<br/>输出: mu_flat<br/>输出shape: B x 10<br/>参考: 第 2 节"]
            cb3["N30 模块: cb3 生成heatmap<br/>输入: attn_output<br/>输入shape: B x 64 x H x W<br/>输出: contact heatmap<br/>输出shape: B x 5 x H x W<br/>参考: 第 2 节"]
            cb_heat["N31 模块: attn=False 的 cb1 + cb2<br/>输入: decoder h<br/>输入shape: B x C x H x W<br/>输出: contact heatmap<br/>输出shape: B x 5 x H x W<br/>参考: 第 2 节"]
            coord["N32 模块: get_coord<br/>输入: contact heatmap<br/>输入shape: B x 5 x H x W<br/>输出: cx/cy坐标分量<br/>输出shape: 每个为 B x 5<br/>参考: 第 2 节"]
            mu["N33 模块: mu 接触点坐标<br/>输入: mu_flat 或 cx/cy坐标分量<br/>输入shape: B x 10 或 2个 B x 5<br/>输出: mu<br/>输出shape: B x 5 x 2<br/>参考: 第 2 节, 第 5 节"]
            varq{"N34 模块: var 开关<br/>输入: fe + var<br/>输入shape: fe为 B x src_in_features，var为布尔值<br/>输出: cvar预测路径或占位路径<br/>输出shape: -<br/>参考: 第 2 节, 第 5 节"}
            varmlp["N35 模块: var_MLP<br/>输入: fe<br/>输入shape: res18 B x 512，res50 B x 2048<br/>输出: cvar<br/>输出shape: B x 5 x 2<br/>参考: 第 2 节, 第 5 节"]
            cvar0["N36 模块: cvar默认占位<br/>输入: var=False + mu<br/>输入shape: 布尔值 + B x 5 x 2<br/>输出: cvar占位<br/>输出shape: 训练为0，推理复用mu B x 5 x 2<br/>参考: 第 2 节, 第 5 节"]
            predtrue["N37 模块: pred_contact var=True<br/>输入: mu + cvar<br/>输入shape: B x 5 x 2 + B x 5 x 2<br/>输出: pred_contact<br/>输出shape: B x 2 x 5 x 2<br/>参考: 第 2 节, 第 5 节"]
            predfalse["N38 模块: pred_contact var=False<br/>输入: mu + mu<br/>输入shape: B x 5 x 2 + B x 5 x 2<br/>输出: pred_contact<br/>输出shape: B x 2 x 5 x 2<br/>参考: 第 2 节, 第 5 节"]
        end

        subgraph traj["轨迹分支 Trajectory Head（见第 2 节与第 5 节）"]
            reshape["N39 模块: fe.reshape<br/>输入: fe<br/>输入shape: res18 B x 512，res50 B x 2048<br/>输出: 单帧单token特征<br/>输出shape: B x 1 x 1 x src_in_features<br/>参考: 第 2 节"]
            down["N40 模块: downproject Linear<br/>输入: 单帧单token特征<br/>输入shape: B x 1 x 1 x src_in_features<br/>输出: 降维token<br/>输出shape: B x 1 x 1 x 256<br/>参考: 第 2 节"]
            encin["N41 模块: encoder_input transpose<br/>输入: 降维token<br/>输入shape: B x 1 x 1 x 256<br/>输出: Encoder输入<br/>输出shape: B x 1 x 1 x 256<br/>参考: 第 2 节"]
            encoder["N42 模块: Transformer Encoder x 6<br/>输入: Encoder输入 + src_mask<br/>输入shape: B x 1 x 1 x 256 + B x 1 x 1<br/>输出: Encoder输出 x<br/>输出shape: B x 1 x 1 x 256<br/>参考: 第 2 节, 第 8.3 节"]
            memory["N43 模块: memory[:,0,:]<br/>输入: Encoder输出 x<br/>输入shape: B x 1 x 1 x 256<br/>输出: context<br/>输出shape: B x 256<br/>参考: 第 2 节"]
            condq{"N44 模块: condition_contact 开关<br/>输入: context + condition_contact<br/>输入shape: B x 256 + 布尔值<br/>输出: 默认context或融合context<br/>输出shape: B x 256<br/>参考: 第 2 节, 第 8.5 节"}
            condfalse["N45 模块: 默认轨迹条件<br/>输入: context<br/>输入shape: B x 256<br/>输出: condition_context<br/>输出shape: B x 256<br/>参考: 第 2 节, 第 8.5 节"]
            condtrue["N46 模块: contact条件融合<br/>输入: context + contact_point<br/>输入shape: B x 256 + B x 2<br/>输出: condition_context<br/>输出shape: B x 256<br/>参考: 第 2 节, 第 8.5 节"]
            hand["N47 模块: TrajAffCVAE / PosVAE<br/>输入: condition_context<br/>输入shape: B x 256<br/>输出: recon_x<br/>输出shape: B x 10<br/>参考: 第 2 节, 第 8.6 节"]
            future["N48 模块: future_hand reshape<br/>输入: recon_x<br/>输入shape: B x 10<br/>输出: future_hand<br/>输出shape: B x 5 x 2<br/>参考: 第 2 节, 第 5 节"]
        end
    end
...
| 开关                  | 默认值        | 图中分支                                                               | 查哪里               |
| ------------------- | ---------- | ------------------------------------------------------------------ | ----------------- |
| `resnet_type`       | `resnet18` | 决定 backbone 全局特征维度和 decoder 通道数                                    | `第 1 节`、`第 6 节`     |
| `attn_kp`           | `True`     | `True` 走空间 attention；`False` 直接把 `cb1/cb2` 输出当 heatmap 解码          | `第 2 节`            |
| `attn_kp_fc`        | `True`     | 只在 `attn_kp=True` 时生效；`True` 走 `fc_mu`，`False` 走 `cb3 + get_coord` | `第 2 节`            |
| `var`               | `False`    | `True` 预测 `cvar`；`False` 训练不算方差，推理用 `[mu, mu]` 占位                  | `第 2 节`、`第 5 节` |
| `condition_contact` | `False`    | 轨迹 CVAE 是否把 `contact_point` 融入 `context`；当前 demo 构造时未启用            | `第 2 节`          |

## 3. Ground Truth、Loss 与总体网络节点对照
...
---
### 8.2 N41 模块 `transpose(1, 2)` 做了什么？

**`transpose(1, 2)`**：**把维度顺序从 `(B, N, T, E)` 调整为 `(B, T, N, E)`**，让后面的 `Encoder` 能正确按 `x.shape[1]=T`、`x.shape[2]=N` 来解析。

### 8.3 N42 模块 `Transformer Encoder x 6` 做了什么？
#### 8.3.1 `x 6` 是什么意思？
就是**把同样的模块摞了 6 层**。

第 1 节写过：`enc_depth = 6`。这个 `Transformer Encoder` 不是一个单一的大黑盒，而是内部有 **6 个 `EncoderBlock`（编码块）串联**。数据从第 1 个块进去，出来后再进第 2 个，一直走到第 6 个，最终输出。
...
**（1）加位置信息** 第 2 节总体网络图里 N42 写得很清楚：即使只有一个 token，模型也会给它打上"patch 位置标签"和"时间位置标签"，告诉模型"这是第 0 个位置、第 0 个时间步的特征"。

**（2）6 层 Self-Attention + MLP 精炼** 每一层 `EncoderBlock`（见第 8.3 节）内部都是这个套路：
...
### 8.5 N44-N46 模块 `condition_contact` 分支做了什么？
#### 8.5.1 N45：确实就是"原样照搬"
...
为什么要有这个"什么都不做"的模块？因为它是**默认路径**。在 VRB 当前的配置里，`condition_contact` 默认是 `False`（第 1 节默认配置没有启用它，且第 5 节变量表可见默认构造时关闭）。这时候模型认为：**预测手部轨迹只需要知道这张图的整体交互语义就够了**，不需要额外告诉它"具体在图像的哪个像素点接触"。
...
### 8.6 N47 模块 `TrajAffCVAE / PosVAE` 做了什么？
#### 8.6.1 N47 本质上是“轨迹生成器”
