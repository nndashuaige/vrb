...
def render_tile(
    ...
    if mode == "objects":
        ...
        header = (
            ...
            f"gap {ref_gap} anchor {anchor_gap} {str(row.get('reference_mode', ''))[:28]}\n"
...
def save_funnel_table(overview: pd.Series, out_path: Path) -> None:
    stages = [
        ...
        ("reference found", int(overview["ref_found"])),
    ...
    table.scale(1, 1.6)
    ax.set_title("E16 排查漏斗", fontsize=14, pad=10)
    plt.tight_layout()
...
def main() -> None:
    ...
    save_funnel_table(overview, SUMMARY_DIR / "e16_debug_funnel_table.png")
    save_contact_sheet(candidate_df, detections, "objects", SUMMARY_DIR / "e16_detected_objects", contact_config)
    save_contact_sheet(candidate_df, detections, "hands", SUMMARY_DIR / "e16_detected_hands", contact_config)
    save_contact_sheet(candidate_df, detections, "contact", SUMMARY_DIR / "e16_contact_frame_contact_points", contact_config)
    save_contact_sheet(candidate_df, detections, "reference", SUMMARY_DIR / "e16_reference_frames", contact_config)
