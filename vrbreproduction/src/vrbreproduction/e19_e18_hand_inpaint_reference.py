"""E19 experiment: E18 hybrid reference plus hand-mask-guided inpainting."""
...
INPAINT_RESULT_DEFAULTS: Dict[str, Any] = {
...
INPAINT_RESULT_FIELDS = tuple(INPAINT_RESULT_DEFAULTS.keys())
...
def get_subactions(config: E19Config) -> pd.DataFrame:
...
def runs_for_subaction(
    ...
    for local_idx, run in enumerate(sorted(runs_in_subaction, key=lambda item: (item["start_frame_0_based"], item["hand"])), start=1):
        ...
        effective_start = max(0, run_start - int(max_ref_backtrack))
        effective_end = run_start - 1
...
def active_object_bbox_and_meta(
    frame_det: Any,
    active_hand: str,
    image_shape: Tuple[int, int],
    score_threshold: float = 0.5,
    ...
    hand_bbox_norm = get_active_hand_bbox(frame_det, active_hand, score_threshold=score_threshold)
    if hand_bbox_norm is None or not hasattr(frame_det, "objects"):
        return meta
    ...
    for obj in frame_det.objects:
        if obj.score >= score_threshold:
            valid_objects.append(
                {
                    "bbox_norm": [obj.bbox.left, obj.bbox.top, obj.bbox.right, obj.bbox.bottom],
                    "score": float(obj.score),
                }
            )
    meta["candidate_count"] = int(len(valid_objects))
    if not valid_objects:
        return meta

    selected_bbox = select_active_object_bbox(hand_bbox_norm, [item["bbox_norm"] for item in valid_objects])
    if selected_bbox is None:
        return meta

    for item in valid_objects:
        if np.allclose(np.asarray(item["bbox_norm"], dtype=float), np.asarray(selected_bbox, dtype=float)):
            meta["bbox_norm"] = list(item["bbox_norm"])
            meta["bbox_px"] = normalized_bbox_to_pixels(selected_bbox, w, h)
            meta["score"] = float(item["score"])
            return meta

    meta["bbox_norm"] = list(selected_bbox)
    meta["bbox_px"] = normalized_bbox_to_pixels(selected_bbox, w, h)
    return meta


class Problem3CachedRunnerE19:
    def __init__(
        self,
        detections,
        image_dir: Path,
        ...
        inpaint_only_if_hand_visible: bool = True,
        inpaint_fail_policy: str = "keep_original",
    ):
        ...
        self.inpaint_only_if_hand_visible = bool(inpaint_only_if_hand_visible)
        self.inpaint_fail_policy = str(inpaint_fail_policy)
    ...
    def build_reference_hand_mask(
        ...
        h, w = ref_img_shape[:2]
        frame_det = self.detections[int(ref_idx)]
        active_meta = hand_bbox_and_meta(frame_det, active_hand, ref_img_shape, score_threshold=0.5)
        bboxes = []
        active_bbox = active_meta.get("bbox_px")
        if active_bbox is not None:
            bboxes.append(active_bbox)
        ...
        if hasattr(frame_det, "hands"):
            for hand in frame_det.hands:
                if hand.score < 0.5:
                    continue
                bbox_norm = [hand.bbox.left, hand.bbox.top, hand.bbox.right, hand.bbox.bottom]
                bbox_px = normalized_bbox_to_pixels(bbox_norm, w, h)
                side = hand.side.name.lower()
                if side != active_hand.lower():
                    bboxes.append(bbox_px)
                all_hand_bboxes.append([float(v) for v in bbox_px])

        mask_u8 = combine_hand_masks(ref_img_shape, bboxes, pad_px=int(pad_px), ellipse=False)
        return mask_u8, {
            "active_hand_masked": bool(active_bbox is not None),
            "any_hand_masked": bool(len(bboxes) > 0),
            "masked_hand_count": int(len(bboxes)),
            "active_hand_bbox_px_for_inpaint": [float(v) for v in active_bbox] if active_bbox is not None else None,
            "all_hand_bboxes_px_for_inpaint": all_hand_bboxes,
            "inpaint_mask_area_ratio": compute_mask_area_ratio(mask_u8),
        }
    ...
    def find_paper_faithful_hybrid_min_iou_reference(
        self,
        *,
        t_contact: int,
        active_hand: str,
        ...
        strict_ref_idx, strict_debug = find_strict_humanless_frame(
            ...
            min_frame_idx=search_start,
        )
        detail: Dict[str, Any] = {
            "reference_attempted_refs": int(max(0, search_end - search_start + 1)),
            "reference_reject_reasons": {},
            "strict_ref_idx_shadow": int(strict_ref_idx) if strict_ref_idx is not None else None,
            "strict_ref_idx": int(strict_ref_idx) if strict_ref_idx is not None else None,
            "strict_reference_anchor_gap_shadow": int(episode_start - int(strict_ref_idx)) if strict_ref_idx is not None else None,
            "strict_reference_anchor_gap": int(episode_start - int(strict_ref_idx)) if strict_ref_idx is not None else None,
            "reference_anchor_gap_delta_vs_strict": 0 if strict_ref_idx is not None else None,
            ...
            "fallback_selected_rank_reason": None,
            "fallback_selected_object_score": None,
            "fallback_selected_distance_to_episode_start": None,
            "fallback_selected_active_contact_binary": None,
            "fallback_selected_any_contact_binary": None,
            "fallback_selected_hand_missing": None,
        ...
        for ref_candidate in range(int(search_end), int(search_start) - 1, -1):
            active_contact = int(left_binary[ref_candidate]) if active_hand == "left" else int(right_binary[ref_candidate])
            any_contact = int(left_binary[ref_candidate] or right_binary[ref_candidate])
            if detail["closest_active_hand_no_contact_frame_shadow"] is None and active_contact == 0:
                detail["closest_active_hand_no_contact_frame_shadow"] = int(ref_candidate)
            if detail["closest_any_contact_zero_frame_shadow"] is None and any_contact == 0:
                detail["closest_any_contact_zero_frame_shadow"] = int(ref_candidate)
            ref_img = self.load_bgr(ref_candidate)
            hand_meta = hand_bbox_and_meta(self.detections[ref_candidate], active_hand, ref_img.shape, score_threshold=0.5)
        ...
        if selected_ref_idx is None:
            ...
            for ref_candidate in range(int(search_end), int(search_start) - 1, -1):
                ...
                hand_meta = hand_bbox_and_meta(
                    self.detections[ref_candidate],
                    active_hand,
                    ref_img.shape,
                    score_threshold=0.5,
                )
                object_meta = active_object_bbox_and_meta(
                    self.detections[ref_candidate],
                    active_hand,
                    ref_img.shape,
                    score_threshold=0.5,
                )
                object_bbox_px = object_meta["bbox_px"]
                if object_bbox_px is None:
                    continue

                hand_bbox_px = hand_meta["bbox_px"]
                hand_missing = hand_bbox_px is None
                hand_object_iou = 0.0 if hand_missing else float(bbox_iou(hand_bbox_px, object_bbox_px))
                object_center_distance = None
                if not hand_missing:
                    object_center_distance = float(bbox_center_distance(hand_bbox_px, object_bbox_px))

                active_contact = int(left_binary[ref_candidate]) if active_hand == "left" else int(right_binary[ref_candidate])
                any_contact = int(left_binary[ref_candidate] or right_binary[ref_candidate])
                distance_to_episode_start = int(episode_start - ref_candidate)
                fallback_candidates.append(
                    {
                        "ref_idx": int(ref_candidate),
                        "hand_missing": bool(hand_missing),
                        "hand_object_iou": float(hand_object_iou),
                        "object_center_distance": object_center_distance,
                        "object_bbox_px": tuple(float(v) for v in object_bbox_px),
                        "object_bbox_score": object_meta["score"],
                        "object_candidate_count": int(object_meta["candidate_count"]),
                        "active_contact": int(active_contact),
                        "any_contact": int(any_contact),
                        "distance_to_episode_start": int(distance_to_episode_start),
                    }
                )

            detail["fallback_candidate_count"] = int(len(fallback_candidates))
            if fallback_candidates:
                fallback_candidates.sort(
                    key=lambda item: (
                        float(item["hand_object_iou"]),
                        int(item["active_contact"]),
                        int(item["any_contact"]),
                        int(item["distance_to_episode_start"]),
                        -float(item["object_bbox_score"]) if item["object_bbox_score"] is not None else 0.0,
                    )
                )
                chosen = fallback_candidates[0]
                selected_ref_idx = int(chosen["ref_idx"])
                selected_via_fallback = True
                detail.update(
                    reference_reject_reasons={"no_active_hand_invisible_reference": 1},
                    reference_hand_object_iou=float(chosen["hand_object_iou"]),
                    reference_hand_object_center_distance=chosen["object_center_distance"],
                    reference_object_bbox=tuple(float(v) for v in chosen["object_bbox_px"]),
                    reference_object_bbox_score=chosen["object_bbox_score"],
                    reference_object_candidate_count=int(chosen["object_candidate_count"]),
                    fallback_selected_rank_reason="min_hand_object_iou_then_contact_then_distance_then_score",
                    fallback_selected_object_score=chosen["object_bbox_score"],
                    fallback_selected_distance_to_episode_start=int(chosen["distance_to_episode_start"]),
                    fallback_selected_active_contact_binary=int(chosen["active_contact"]),
                    fallback_selected_any_contact_binary=int(chosen["any_contact"]),
                    fallback_selected_hand_missing=bool(chosen["hand_missing"]),
                )
            else:
                detail["reference_reject_reasons"] = {
                    "no_active_hand_invisible_reference": 1,
                    "no_min_hand_object_iou_fallback_reference": 1,
                }
                return None, detail
        ...
        if detail.get("reference_object_bbox") is None:
            object_meta = active_object_bbox_and_meta(self.detections[ref_idx], active_hand, ref_img.shape, score_threshold=0.5)
            if object_meta["bbox_px"] is not None:
                detail["reference_object_bbox"] = tuple(float(v) for v in object_meta["bbox_px"])
            detail["reference_object_bbox_score"] = object_meta["score"]
            detail["reference_object_candidate_count"] = int(object_meta["candidate_count"])
        detail.update(
            reference_mode=(
                "min_hand_object_iou_pre_contact_fallback"
                if selected_via_fallback
                else "active_hand_invisible_pre_contact"
            ),
            reference_reason=(
                "min_hand_object_iou_pre_contact_fallback"
                if selected_via_fallback
                else "closest_pre_contact_active_hand_invisible"
            ),
        ...
        if strict_ref_idx is not None:
            detail["reference_anchor_gap_delta_vs_strict"] = int((episode_start - ref_idx) - (episode_start - int(strict_ref_idx)))
    ...
    def run_cell3(
        ...
        *,
        ...
        if search_end < search_start:
            result.update(
                discard=True,
                discard_reason="no_e18_reference",
                status="DISCARD",
                no_pre_episode_reference_window_not_rescued=True,
                reference_mode="no_e18_reference",
                reference_reason="no_e18_reference",
                fallback_reference_attempted=True,
            )
            return result
        ...
        ref_idx, detail = self.find_paper_faithful_hybrid_min_iou_reference(
        ...
        if ref_idx is None:
            ...
            return result
        fallback_used = result.get("reference_mode") == "min_hand_object_iou_pre_contact_fallback"
        result.update(
            fallback_reference_used=bool(fallback_used),
            fallback_reference_attempted=bool(fallback_used or result.get("fallback_candidate_count", 0) > 0),
            fallback_reference_in_pre_contact_gap=bool(fallback_used),
            fallback_ref_before_episode_start=bool(fallback_used and int(ref_idx) < episode_start),
            rescued_from_no_strict_humanless=bool(fallback_used and result.get("strict_ref_idx_shadow") is None),
            fallback_rescued_from_no_strict_humanless=bool(fallback_used and result.get("strict_ref_idx_shadow") is None),
        ...
        result.update(
            inpaint_backend=self.inpaint_config.backend,
            inpaint_fail_policy=self.inpaint_fail_policy,
            inpaint_active_hand_masked=bool(mask_meta["active_hand_masked"]),
            inpaint_any_hand_masked=bool(mask_meta["any_hand_masked"]),
            inpaint_masked_hand_count=int(mask_meta["masked_hand_count"]),
            inpaint_mask_area_ratio=float(mask_meta["inpaint_mask_area_ratio"]),
            active_hand_bbox_px_for_inpaint=mask_meta["active_hand_bbox_px_for_inpaint"],
            all_hand_bboxes_px_for_inpaint=mask_meta["all_hand_bboxes_px_for_inpaint"],
        )
        ...
        inpaint_required = bool(result.get("active_hand_visible_at_ref")) or not self.inpaint_only_if_hand_visible
        result["inpaint_required"] = bool(inpaint_required)
        result["inpaint_fail_policy"] = self.inpaint_fail_policy
        if inpaint_required:
            result["inpaint_attempted"] = True
            if int(np.count_nonzero(mask_u8 > 0)) == 0:
                result["inpaint_status"] = "failed_kept_original"
                result["inpaint_fail_reason"] = "empty_hand_mask"
            else:
                original_path = output_dir / "reference_frame_original.png"
                mask_path = output_dir / "reference_hand_mask.png"
                inpainted_path = output_dir / "reference_frame_inpainted.png"
                before_after_path = output_dir / "reference_inpaint_before_after.png"
                cv2.imwrite(str(original_path), ref_img)
                cv2.imwrite(str(mask_path), mask_u8)
                result["inpaint_reference_frame_original"] = str(original_path)
                result["inpaint_mask_path"] = str(mask_path)
                try:
                    ...
                    result["inpaint_boundary_delta"] = float(compute_boundary_delta(ref_img, inpainted_bgr, mask_u8, band_px=5))
                    cv2.imwrite(str(inpainted_path), inpainted_bgr)
                    mask_rgb = cv2.cvtColor(mask_u8, cv2.COLOR_GRAY2BGR)
                    before_after = np.concatenate([ref_img, mask_rgb, inpainted_bgr], axis=1)
                    cv2.imwrite(str(before_after_path), before_after)
                    result["inpaint_reference_frame"] = str(inpainted_path)
                    result["inpaint_before_after_path"] = str(before_after_path)
                    fail_reasons = []
                    if float(result["inpaint_mask_area_ratio"] or 0.0) > float(self.inpaint_config.max_mask_area_ratio):
                        fail_reasons.append("mask_area_ratio_too_large")
                    if float(result["inpaint_boundary_delta"] or 0.0) > float(self.inpaint_config.boundary_delta_max):
                        fail_reasons.append("boundary_delta_too_large")
                    if fail_reasons:
                        result["inpaint_fail_reason"] = ";".join(fail_reasons)
                        if self.inpaint_fail_policy == "discard":
                            result.update(
                                discard=True,
                                discard_reason=f"inpaint_quality_gate_failed:{result['inpaint_fail_reason']}",
                                status="DISCARD",
                                inpaint_status="discarded",
                                inpaint_applied=False,
                            )
                            return result
                        result["inpaint_status"] = "failed_kept_original"
                        result["inpaint_applied"] = False
                    else:
                        ref_img_visual = inpainted_bgr
                        result["inpaint_status"] = "applied"
                        result["inpaint_applied"] = bool(inpaint_meta.get("inpaint_applied", True))
                except Exception as exc:
                    result["inpaint_fail_reason"] = str(exc)
                    if self.inpaint_fail_policy == "discard":
                        result.update(
                            discard=True,
                            discard_reason=f"inpaint_quality_gate_failed:{exc}",
                            status="DISCARD",
                            inpaint_status="discarded",
                            inpaint_applied=False,
                        )
            ...
            result["inpaint_status"] = "not_required"

        result["ref_img_visual_bgr"] = ref_img_visual
        result["status"] = "KEEP"
        return result
...
def write_cell4_pipeline_outputs(
    *,
    ...
    ref_img_bgr = cell3_result.get("ref_img_visual_bgr", ref_img_bgr)
...
def base_candidate_record(
    *,
    ...
    return {
        ...
        "max_ref_backtrack": int(candidate["max_ref_backtrack"]) if candidate.get("max_ref_backtrack") is not None else None,
...
def run_one_candidate(
    *,
    ...
    problem3_runner: Problem3CachedRunnerE19,
    config: E19Config,
    subaction_new_contact_count: int,
    raw_candidate_count: int,
) -> Dict[str, Any]:
    ...
    if cell3.get("discard") or cell3.get("status") != "KEEP":
        ...
        failed_stage = "reference_selection" if reason == "no_e18_reference" else "Cell3_homography_geometry"
    ...
    cell3.pop("ref_img_visual_bgr", None)
...
def build_overview(candidate_df: pd.DataFrame, subaction_summary_df: pd.DataFrame, candidate_plan_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def bool_count(column: str) -> int:
        if column not in candidate_df:
            return 0
        return int(candidate_df[column].fillna(False).astype(bool).sum())

    def mode_count(column: str, mode: str, keep_only: bool = False) -> int:
        if column not in candidate_df:
            return 0
        mask = candidate_df[column] == mode
        if keep_only and "status" in candidate_df:
            mask = mask & (candidate_df["status"] == "keep")
        return int(mask.sum())

    active_invisible = candidate_df.get("reference_mode", pd.Series(dtype=object)) == "active_hand_invisible_pre_contact"
    fallback = candidate_df.get("reference_mode", pd.Series(dtype=object)) == "min_hand_object_iou_pre_contact_fallback"
    keep = candidate_df.get("status", pd.Series(dtype=object)) == "keep"
    return pd.DataFrame(
        [
            {
                "experiment_id": EXPERIMENT["experiment_id"],
                ...
                "strict_ref_found": int(candidate_df.get("strict_ref_idx", pd.Series(dtype=object)).notna().sum()) if "strict_ref_idx" in candidate_df else 0,
                "active_hand_invisible_ref_found": int((candidate_df.get("reference_mode") == "active_hand_invisible_pre_contact").sum()) if "reference_mode" in candidate_df else 0,
                "fallback_ref_found": int((candidate_df.get("reference_mode") == "min_hand_object_iou_pre_contact_fallback").sum()) if "reference_mode" in candidate_df else 0,
                "clean_ref_found": 0,
                "active_hand_invisible_ref_keep": int(((candidate_df["status"] == "keep") & (candidate_df.get("reference_mode") == "active_hand_invisible_pre_contact")).sum()) if "reference_mode" in candidate_df else 0,
                "fallback_ref_keep": int(((candidate_df["status"] == "keep") & (candidate_df.get("reference_mode") == "min_hand_object_iou_pre_contact_fallback")).sum()) if "reference_mode" in candidate_df else 0,
                "inpaint_required": bool_count("inpaint_required"),
                "inpaint_attempted": bool_count("inpaint_attempted"),
                "inpaint_applied": bool_count("inpaint_applied"),
                "inpaint_failed_kept_original": mode_count("inpaint_status", "failed_kept_original"),
                "inpaint_discarded": mode_count("inpaint_status", "discarded"),
                "active_hand_visible_ref_keep": int((keep & candidate_df.get("active_hand_visible_at_ref", pd.Series(dtype=bool)).fillna(False).astype(bool)).sum()) if len(candidate_df) else 0,
                "active_hand_visible_ref_inpaint_applied_keep": int((keep & candidate_df.get("active_hand_visible_at_ref", pd.Series(dtype=bool)).fillna(False).astype(bool) & candidate_df.get("inpaint_applied", pd.Series(dtype=bool)).fillna(False).astype(bool)).sum()) if len(candidate_df) else 0,
                "inpaint_required_active_hand_invisible_ref": int((active_invisible & candidate_df.get("inpaint_required", pd.Series(dtype=bool)).fillna(False).astype(bool)).sum()) if len(candidate_df) else 0,
                "inpaint_applied_active_hand_invisible_ref": int((active_invisible & candidate_df.get("inpaint_applied", pd.Series(dtype=bool)).fillna(False).astype(bool)).sum()) if len(candidate_df) else 0,
                "inpaint_required_fallback_ref": int((fallback & candidate_df.get("inpaint_required", pd.Series(dtype=bool)).fillna(False).astype(bool)).sum()) if len(candidate_df) else 0,
                "inpaint_applied_fallback_ref": int((fallback & candidate_df.get("inpaint_applied", pd.Series(dtype=bool)).fillna(False).astype(bool)).sum()) if len(candidate_df) else 0,
...
def save_bar_chart_failure_reasons(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E19 failure reasons")
...
def save_funnel_chart(overview_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("count")
    ax.set_title("E19 pipeline funnel")
...
def save_timeline_chart(candidate_df: pd.DataFrame, subactions: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("Timeline: E19 first-contact samples and reference arrows")
...
def save_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E19 success ref_gap distribution")
...
def save_candidate_offset_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E19 success candidate offset distribution")
...
def save_gap_length_histogram(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E19 pre-contact gap length distribution")
...
def save_reference_mode_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(8, 4))
    for mode, color in [
        ("active_hand_invisible_pre_contact", "#1a73e8"),
    ]:
    ...
    ax.set_title("E19 success ref_gap by reference mode")
    ax.set_xlabel("candidate frame - reference frame")
    ax.set_ylabel("count")
    ax.legend()
    plt.tight_layout()
...
def save_overlay_contact_sheet(df: pd.DataFrame, path: Path, title: str, max_items: int = 30) -> None:
    ...
    for _, row in view.iterrows():
        ...
        labels.append(
            ...
            f"f{int(row['frame_0_based'])} ref{int(row['ref_idx'])} gap{int(row['ref_gap'])} "
...
def save_inpaint_contact_sheet(candidate_df: pd.DataFrame, charts_dir: Path, per_page: int = 24) -> None:
    if candidate_df.empty or "inpaint_attempted" not in candidate_df:
        return
    view = candidate_df[candidate_df["inpaint_attempted"].fillna(False).astype(bool)].copy()
    if view.empty:
        return
    view = view.sort_values(["subaction_index", "frame_0_based"])
    charts_dir.mkdir(parents=True, exist_ok=True)

    def read_rgb(path_value: Any) -> Optional[np.ndarray]:
    ...
    samples_per_page = int(max(1, per_page))
    for page_idx, start in enumerate(range(0, len(view), samples_per_page), start=1):
        page = view.iloc[start:start + samples_per_page]
        tiles = []
        labels = []
        for _, row in page.iterrows():
            imgs = [
                read_rgb(row.get("inpaint_reference_frame_original")),
                read_rgb(row.get("inpaint_mask_path")),
                read_rgb(row.get("inpaint_reference_frame")),
                read_rgb(row.get("label_heatmap_overlay")),
            ]
            fallback = np.zeros((thumb_h, thumb_w, 3), dtype=np.uint8)
            thumbs = []
            for img in imgs:
                if img is None:
                    thumbs.append(fallback.copy())
                    continue
                if img.ndim == 2:
                    img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
                thumbs.append(cv2.resize(img, (thumb_w, thumb_h), interpolation=cv2.INTER_AREA))
            tiles.append(np.concatenate(thumbs, axis=1))
            boundary = row.get("inpaint_boundary_delta")
            boundary_text = "" if pd.isna(boundary) else f"{float(boundary):.1f}"
            labels.append(
                f"s{int(row.get('subaction_index', -1))} {row.get('narration_id', '')} "
                f"{row.get('hand', '')} ref{row.get('ref_idx', '')}\n"
                f"{row.get('inpaint_status', '')} delta={boundary_text}"
            )
        if not tiles:
            continue
        cols = min(2, len(tiles))
        rows = int(np.ceil(len(tiles) / cols))
        fig, axes = plt.subplots(rows, cols, figsize=(cols * 8.0, rows * 2.2))
        axes_arr = np.asarray(axes).reshape(-1)
        for ax in axes_arr:
            ax.axis("off")
        for ax, tile, label in zip(axes_arr, tiles, labels):
            ax.imshow(tile)
            ax.set_title(label, fontsize=7)
        fig.suptitle("E19 hand inpainting: original | mask | inpainted | heatmap overlay", fontsize=12)
        plt.tight_layout()
        out_path = charts_dir / f"e19_inpaint_contact_sheet_page_{page_idx:03d}.png"
...
def write_comparison_files(candidate_df: pd.DataFrame, output_root: Path) -> None:
    ...
    for baseline_exp, baseline_root_name, out_name in [
        ("E6b", "数据处理小批量测试前100个subaction_E6b_pre_contact_gap_crop_fallback", "e19_success_vs_e6b.csv"),
        ("E6c", "数据处理小批量测试前100个subaction_E6c_object_consistency", "e19_success_vs_e6c.csv"),
    ]:
    ...
    *,
    ...
    for idx, row in compare_overview.iterrows():
        if not row.get("reference_anchor_gap_median_keep"):
            ...
            baseline_root = resolve_output_dir("e15") if exp_id == "E15" else None
            if exp_id == "E14":
    ...
    lines = [
        "# E19 E18-hybrid hand-inpaint reference - 诊断实验报告",
        "",
        ...
        "- E19 先在原始帧上计算 homography、projected contact points、heatmap 参数和 trajectory 坐标；手部 inpainting 只用于最终 plausible hand-free reference canvas 上重绘标签。",
        "",
        "## Funnel",
        "",
        markdown_table(
        ...
        "## Hand Inpainting",
        "",
        f"- backend: `{config.inpaint_backend}`",
        f"- mask pad px: `{config.inpaint_mask_pad_px}`",
        f"- fail policy: `{config.inpaint_fail_policy}`",
        f"- inpaint required / attempted / applied: `{int(overview_df.iloc[0].get('inpaint_required', 0))}` / `{int(overview_df.iloc[0].get('inpaint_attempted', 0))}` / `{int(overview_df.iloc[0].get('inpaint_applied', 0))}`",
        f"- failed kept original: `{int(overview_df.iloc[0].get('inpaint_failed_kept_original', 0))}`",
        f"- discarded by inpaint gate: `{int(overview_df.iloc[0].get('inpaint_discarded', 0))}`",
        f"- active-hand-visible keep before/after inpaint: `{int(overview_df.iloc[0].get('active_hand_visible_ref_keep', 0))}` / `{int(overview_df.iloc[0].get('active_hand_visible_ref_inpaint_applied_keep', 0))}`",
        f"- median mask area ratio: `{distribution_stats(candidate_df['inpaint_mask_area_ratio']).get('median') if 'inpaint_mask_area_ratio' in candidate_df else None}`",
        f"- median boundary delta: `{distribution_stats(candidate_df['inpaint_boundary_delta']).get('median') if 'inpaint_boundary_delta' in candidate_df else None}`",
        "",
        "E19 keeps E18's geometry path comparable by computing homography, projected contact points, heatmap parameters, and trajectory coordinates on the original frames. Hand inpainting is applied only after these coordinates are cached, and the final heatmap/trajectory overlays are redrawn on the hand-free reference canvas. Inpainting statistics are reported separately for `active_hand_invisible_pre_contact` and `min_hand_object_iou_pre_contact_fallback` reference modes. Inpainting produces a plausible hand-free reference, not the true hidden surface.",
        "",
        markdown_table(
            overview_df,
            [
        ...
        f"- active_hand_invisible_ref_found / fallback_ref_found / fallback_ref_keep: `{int(overview_df.iloc[0].get('active_hand_invisible_ref_found', 0))}` / `{int(overview_df.iloc[0].get('fallback_ref_found', 0))}` / `{int(overview_df.iloc[0].get('fallback_ref_keep', 0))}`",
        ...
        "## success 分布",
        "",
        f"- active-hand-invisible ref_gap stats: `{distribution_stats(success_df['ref_gap']) if 'ref_gap' in success_df else distribution_stats(pd.Series(dtype=float))}`",
        ...
        f"- fallback selected hand-object IoU stats: `{distribution_stats(candidate_df[candidate_df.get('reference_mode') == 'min_hand_object_iou_pre_contact_fallback']['reference_hand_object_iou']) if 'reference_hand_object_iou' in candidate_df else distribution_stats(pd.Series(dtype=float))}`",
        f"- strict shadow reference anchor gap stats: `{distribution_stats(candidate_df['strict_reference_anchor_gap']) if 'strict_reference_anchor_gap' in candidate_df else distribution_stats(pd.Series(dtype=float))}`",
        ...
        "## 图表",
        "",
        "- pipeline funnel: `charts/e19_pipeline_funnel.png`",
        "- failure reasons: `charts/e19_failure_reasons.png`",
        "- success ref_gap histogram: `charts/e19_ref_gap_success_hist.png`",
        "- success reference_anchor_gap histogram: `charts/e19_reference_anchor_gap_hist.png`",
        "- success contact sheet: `charts/e19_success_contact_sheet_page_001.png`",
        "- success heatmap+trajectory contact sheet: `charts/e19_success_heatmap_trajectory_contact_sheet_page_001.png`",
        "",
        "## 明细文件",
        "",
        ...
        "- E18 vs E6b success table: `e19_success_vs_e6b.csv`",
        "- E18 vs E6c success table: `e19_success_vs_e6c.csv`",
        "- pipeline outputs: `experiments/E19/pipeline_outputs/`",
    ]
...
def save_reference_anchor_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E19 success reference_anchor_gap distribution")
...
def run_e19_experiment(config: Optional[E19Config] = None) -> Dict[str, Any]:
    config = config or E19Config()
    ...
    inpaint_config = InpaintConfig(
        ...
        mask_pad_px=config.inpaint_mask_pad_px,
        opencv_radius=config.inpaint_opencv_radius,
        lama_command=config.inpaint_lama_command,
        sd_command=config.inpaint_sd_command,
        max_mask_area_ratio=config.inpaint_max_mask_area_ratio,
        boundary_delta_max=config.inpaint_boundary_delta_max,
    )
    problem3_runner = Problem3CachedRunnerE19(
        detections,
        config.image_dir,
        ...
        inpaint_only_if_hand_visible=config.inpaint_only_if_hand_visible,
        inpaint_fail_policy=config.inpaint_fail_policy,
    )
    ...
    print(
        f"Inpaint backend: {config.inpaint_backend} | mask_pad={config.inpaint_mask_pad_px} | "
        f"fail_policy={config.inpaint_fail_policy}"
    )
    ...
    for idx, row in subactions.iterrows():
        ...
        completed = int(idx) + 1
        progress_pct = 100.0 * completed / max(1, len(subactions))
        print(
            f"[E19 progress {completed}/{len(subactions)} {progress_pct:5.1f}%] "
            f"subaction {idx:02d} {row['narration_id']} | {row['narration']} | "
            ...
            _json_safe(
                {
                    ...
                    "inpaint": {
                        ...
                        "mask_pad_px": config.inpaint_mask_pad_px,
                        "opencv_radius": config.inpaint_opencv_radius,
                        "only_if_hand_visible": config.inpaint_only_if_hand_visible,
                        "fail_policy": config.inpaint_fail_policy,
                        "max_mask_area_ratio": config.inpaint_max_mask_area_ratio,
                        "boundary_delta_max": config.inpaint_boundary_delta_max,
                    },
    ...
    save_funnel_chart(overview_df, charts_dir / "e19_pipeline_funnel.png")
    save_bar_chart_failure_reasons(candidate_df, charts_dir / "e19_failure_reasons.png")
    save_timeline_chart(candidate_df, subactions, charts_dir / "e19_timeline.png")
    success_df = candidate_df[candidate_df["status"] == "keep"].copy()
    save_ref_gap_histogram(success_df, charts_dir / "e19_ref_gap_success_hist.png")
    save_reference_anchor_gap_histogram(success_df, charts_dir / "e19_reference_anchor_gap_hist.png")
    save_candidate_offset_histogram(success_df, charts_dir / "e19_candidate_offset_success_hist.png")
    save_gap_length_histogram(candidate_df, charts_dir / "e19_gap_length_hist.png")
    save_reference_mode_ref_gap_histogram(success_df, charts_dir / "e19_ref_gap_success_by_reference_mode_hist.png")
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e19_success_contact_sheet",
        "E19 success samples",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e19_success_contact_sheet_page_001.png",
        "E19 success samples",
    )
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e19_success_heatmap_trajectory_contact_sheet",
        "E19 success heatmap + trajectory overlays",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e19_success_heatmap_trajectory_contact_sheet_page_001.png",
        "E19 success heatmap + trajectory overlays",
    )
    save_inpaint_contact_sheet(candidate_df[candidate_df.get("inpaint_attempted", False) == True], charts_dir, per_page=24)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run E19 E18-hybrid reference plus hand inpainting experiment.")
    parser.add_argument("--video-id", default=E19Config.video_id)
    parser.add_argument("--num-subactions", type=int, default=E19Config.num_subactions)
    parser.add_argument("--max-ref-backtrack", type=int, default=E19Config.max_ref_backtrack)
    parser.add_argument("--crop-size", type=int, default=E19Config.crop_size)
    parser.add_argument("--output-root", type=Path, default=E19Config.output_root)
...
if __name__ == "__main__":
    ...
    run_e19_experiment(
        E19Config(
            ...
            inpaint_mask_pad_px=args.inpaint_mask_pad_px,
            inpaint_opencv_radius=args.inpaint_opencv_radius,
            inpaint_fail_policy=args.inpaint_fail_policy,
            inpaint_lama_command=args.inpaint_lama_command,
            inpaint_sd_command=args.inpaint_sd_command,
            inpaint_max_mask_area_ratio=args.inpaint_max_mask_area_ratio,
            inpaint_boundary_delta_max=args.inpaint_boundary_delta_max,
        )
    )
