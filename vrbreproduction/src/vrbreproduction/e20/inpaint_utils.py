...
# --- E20-C additions -------------------------------------------------------
# E20-B masks swallowed the interaction target in 20/56 samples
# (overlap_heatmap_top >= 0.99): HOA/DINO hand boxes often contain the held
# object, SAM segments "the salient thing in the box", and bridging/dilation
# closes the gap. The helpers below support a label-geometry protect mask that
# is subtracted from the inpaint mask as a hard constraint.


def disk_mask(image_hw: Sequence[int], points: Iterable[Sequence[float]], radius: int = 6) -> np.ndarray:
    """Filled disks at the given (x, y) points; used to protect contact points."""
    h, w = int(image_hw[0]), int(image_hw[1])
    mask = np.zeros((h, w), dtype=np.uint8)
    for pt in points or []:
        ...
        if not (np.isfinite(x) and np.isfinite(y)):
            continue
        cx = int(round(np.clip(x, 0, w - 1)))
        cy = int(round(np.clip(y, 0, h - 1)))
        cv2.circle(mask, (cx, cy), int(max(1, radius)), 255, -1)
    return mask


def mask_intersection_frac(mask_a: np.ndarray, mask_b: np.ndarray) -> float:
    """|a ∩ b| / |a| (0 when a is empty)."""
    a = mask_a > 0
    denom = int(np.count_nonzero(a))
    if denom == 0:
        return 0.0
    return float(np.count_nonzero(a & (mask_b > 0)) / denom)


def sharpness_ratio_vs_original(edited: np.ndarray, original: np.ndarray, mask_u8: np.ndarray) -> float:
    """In-mask Laplacian-variance ratio edited/original.

    Calibrated on the 56 E20-B samples: every visually hallucinated SD final
    (new textures/text in the hole) scored > 2.0, every clean SD final <= 1.95,
    and every LaMa result < 1.0. Used as the SD hallucination gate.
    """
    return float(laplacian_variance(edited, mask_u8) / max(laplacian_variance(original, mask_u8), 1e-6))
