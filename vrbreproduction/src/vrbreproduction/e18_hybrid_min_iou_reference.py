"""E18 experiment: hybrid active-hand-invisible plus min-IoU fallback reference."""
...
class E18Config:
    ...
    output_root: Path = VRBREPRODUCTION_ROOT / "outputs" / "e18"
...
EXPERIMENT = {
    "experiment_id": "E18",
    "name": "E18_hybrid_min_iou_reference",
    ...
    "description": (
        ...
        "reference 在 episode start 之前搜索，最多向前回看 max_ref_backtrack 帧；"
...
def get_subactions(config: E18Config) -> pd.DataFrame:
...
def run_e18_experiment(config: Optional[E18Config] = None) -> Dict[str, Any]:
    config = config or E18Config()
    ...
    for idx, row in subactions.iterrows():
        ...
        print(
            f"E18 subaction {idx:02d} {row['narration_id']} | {row['narration']} | "
    ...
    save_timeline_chart(candidate_df, subactions, charts_dir / "e18_timeline.png")
    success_df = candidate_df[candidate_df["status"] == "keep"].copy()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run E18 hybrid active-hand-invisible plus min-IoU fallback reference baseline.")
    parser.add_argument("--video-id", default=E18Config.video_id)
    parser.add_argument("--num-subactions", type=int, default=E18Config.num_subactions)
    parser.add_argument("--max-ref-backtrack", type=int, default=E18Config.max_ref_backtrack)
    parser.add_argument("--crop-size", type=int, default=E18Config.crop_size)
    parser.add_argument("--output-root", type=Path, default=E18Config.output_root)
