"""E14 experiment: closest clean pre-contact reference."""
...
class E14Config:
    ...
    hand_object_iou_max: float = 0.05
    min_hand_object_center_distance_px: Optional[float] = None
    output_root: Path = VRBREPRODUCTION_ROOT / "outputs" / "e14"
...
EXPERIMENT = {
    "experiment_id": "E14",
    "name": "E14_closest_clean_pre_contact_reference",
    "contact_strategy": "first_new_contact_episode_first_timestep",
    "reference_strategy": "closest_clean_pre_contact_reference",
    "reference_fallback": "none",
    "description": (
        "每个 subaction 只取第一个 smoothed contact 0->1 episode 的 first contact timestep；"
        "reference 从该 episode 前的 contiguous no-contact gap 中向后扫描，选择最近且 clean 的 pre-contact frame；"
        "clean gate 要求 active hand 未 contact、contact crop 手遮挡低、手和投影目标物 IoU 低，并保留 strict humanless shadow 诊断。"
    ),
}
...
def get_subactions(config: E14Config) -> pd.DataFrame:
...
class Problem3CachedRunnerE14:
    def __init__(
        ...
        self.hand_object_iou_max = float(hand_object_iou_max)
    ...
    def find_closest_clean_pre_contact_reference(
        self,
        *,
        t_contact: int,
        active_hand: str,
        ...
        episode_start: int,
        crop_size: int,
        crop_hand_overlap_max: float,
        hand_object_iou_max: float,
        min_hand_object_center_distance_px: Optional[float],
    ) -> Tuple[Optional[int], Dict[str, Any]]:
        reject_reasons: Counter = Counter()
        attempted = 0
        strict_ref_idx, _ = find_strict_humanless_frame(
            self.detections,
            episode_start,
            score_threshold=0.5,
        ...
        best_detail: Dict[str, Any] = {
            ...
            "projected_contact_centroid_in_bounds": False,
        ...
        for ref_candidate in range(int(search_end), int(search_start) - 1, -1):
            attempted += 1
            if ref_candidate >= int(episode_start):
                reject_reasons["ref_not_before_episode_start"] += 1
                continue
            ...
            if active_contact != 0:
                reject_reasons["active_hand_contact_binary_not_zero"] += 1
                continue
            H_contact_to_ref, pair_stats, fail_reason = self.accumulate_homography_to_ref(ref_candidate, int(t_contact))
            ratios = [float(item.get("inlier_ratio", 0.0)) for item in pair_stats if item.get("inlier_ratio") is not None]
            inliers = [int(item.get("inliers", 0)) for item in pair_stats if item.get("inliers") is not None]
            good_matches = [int(item.get("good_matches", 0)) for item in pair_stats if item.get("good_matches") is not None]
            if ratios:
                current_min_ratio = min(ratios)
                if (
                    best_detail["fallback_precheck_homography_min_inlier_ratio"] is None
                    or current_min_ratio > best_detail["fallback_precheck_homography_min_inlier_ratio"]
                ):
                    best_detail["fallback_precheck_homography_min_inlier_ratio"] = current_min_ratio
                    best_detail["fallback_precheck_homography_min_inliers"] = min(inliers) if inliers else None
                    best_detail["fallback_precheck_homography_min_good_matches"] = min(good_matches) if good_matches else None
            if H_contact_to_ref is None:
                reject_reasons[fail_reason or "homography_failed"] += 1
                continue
            projected_object_bbox_ref = None
            if projected_object_bbox_contact is not None:
                projected_object_polygon = transform_bbox_to_polygon(
                    np.asarray(projected_object_bbox_contact, dtype=np.float32),
                    H_contact_to_ref,
                )
                projected_object_bbox_ref = polygon_to_bbox(projected_object_polygon, self.load_gray(ref_candidate).shape)

            projected = transform_points(np.asarray(contact_means, dtype=np.float32), H_contact_to_ref)
            if projected is None or len(projected) == 0 or not np.isfinite(projected).all():
                reject_reasons["projected_contact_invalid"] += 1
                continue
            ref_img = self.load_bgr(ref_candidate)
            centroid = projected.mean(axis=0)
            h_img, w_img = ref_img.shape[:2]
            if centroid[0] < 0 or centroid[1] < 0 or centroid[0] > w_img or centroid[1] > h_img:
                reject_reasons["projected_contact_centroid_out_of_bounds"] += 1
                continue

            crop_bbox = compute_centered_crop_bbox(centroid, ref_img.shape, crop_size=int(crop_size))
            overlap = compute_crop_hand_overlap_ratio(
                self.detections[int(ref_candidate)],
                ref_img.shape,
                crop_bbox,
                score_threshold=0.5,
            )
            if float(overlap) > float(crop_hand_overlap_max):
                reject_reasons["crop_hand_overlap_above_threshold"] += 1
                continue

            hand_meta = hand_bbox_and_meta(self.detections[int(ref_candidate)], active_hand, ref_img.shape, score_threshold=0.5)
            object_candidates = []
            if hasattr(self.detections[int(ref_candidate)], "objects"):
                for obj in self.detections[int(ref_candidate)].objects:
                    if obj.score < 0.5:
                        continue
                    bbox = (
                        float(obj.bbox.left) * w_img,
                        float(obj.bbox.top) * h_img,
                        float(obj.bbox.right) * w_img,
                        float(obj.bbox.bottom) * h_img,
                    )
                    object_candidates.append({"bbox": bbox, "score": float(obj.score)})

            best_object = None
            if projected_object_bbox_ref is not None and object_candidates:
                best_key = None
                for item in object_candidates:
                    iou = bbox_iou(projected_object_bbox_ref, item["bbox"])
                    center_dist = bbox_center_distance(projected_object_bbox_ref, item["bbox"])
                    key = (iou, -center_dist, item["score"])
            ...
            hand_object_iou = None
            hand_object_center_distance = None
            target_bbox_for_hand = best_object["bbox"] if best_object is not None else projected_object_bbox_ref
            if hand_meta["bbox_px"] is not None and target_bbox_for_hand is not None:
                hand_object_iou = float(bbox_iou(hand_meta["bbox_px"], target_bbox_for_hand))
                hand_object_center_distance = float(bbox_center_distance(hand_meta["bbox_px"], target_bbox_for_hand))
                if hand_object_iou > float(hand_object_iou_max):
                    reject_reasons["hand_object_iou_above_threshold"] += 1
                    continue
                if (
                    min_hand_object_center_distance_px is not None
                    and hand_object_center_distance < float(min_hand_object_center_distance_px)
                ):
                    reject_reasons["hand_object_center_distance_below_threshold"] += 1
                    continue

            mode = "closest_clean_pre_contact_active_hand_visible" if hand_meta["visible"] else "closest_clean_pre_contact_humanless"
            detail = {
                "reference_attempted_refs": int(attempted),
                "reference_reject_reasons": dict(reject_reasons),
                "crop_bbox_150": tuple(int(v) for v in crop_bbox),
                "crop_hand_overlap_ratio": float(overlap),
                "projected_contact_centroid_ref": (float(centroid[0]), float(centroid[1])),
                "projected_contact_centroid_in_bounds": True,
                ...
                "reference_hand_object_iou": hand_object_iou,
                "reference_hand_object_center_distance": hand_object_center_distance,
                "reference_object_bbox": tuple(float(v) for v in best_object["bbox"]) if best_object is not None else None,
                "reference_object_bbox_score": float(best_object["score"]) if best_object is not None else None,
                "reference_object_iou": float(best_object["iou"]) if best_object is not None else None,
                "reference_object_center_distance": float(best_object["center_distance"]) if best_object is not None else None,
                "projected_object_bbox": tuple(float(v) for v in projected_object_bbox_ref) if projected_object_bbox_ref is not None else None,
                "fallback_precheck_homography_min_inlier_ratio": min(ratios) if ratios else None,
                "fallback_precheck_homography_min_inliers": min(inliers) if inliers else None,
                "fallback_precheck_homography_min_good_matches": min(good_matches) if good_matches else None,
                "reference_mode": mode,
                "reference_reason": "closest_clean_pre_contact_frame",
            }
            anchor_gap = int(episode_start - int(ref_candidate))
            if strict_ref_idx is not None:
                detail["reference_anchor_gap_delta_vs_strict"] = int(anchor_gap - (episode_start - int(strict_ref_idx)))
            return int(ref_candidate), detail

        best_detail["reference_attempted_refs"] = int(attempted)
        best_detail["reference_reject_reasons"] = dict(reject_reasons)
    ...
    def run_cell3(
        ...
        *,
        ...
        result["fallback_reference_attempted"] = True
        ref_idx, detail = self.find_closest_clean_pre_contact_reference(
            ...
            crop_hand_overlap_max=float(crop_hand_overlap_max),
            hand_object_iou_max=float(hand_object_iou_max if hand_object_iou_max is not None else self.hand_object_iou_max),
        ...
        if ref_idx is None:
            result.update(
                discard=True,
                discard_reason="no_clean_pre_contact_reference",
                status="DISCARD",
                reference_mode="no_clean_pre_contact_reference",
                reference_reason="clean_reference_search_failed",
            )
            return result
        result.update(
            fallback_reference_used=False,
...
def run_one_candidate(
    *,
    ...
    problem3_runner: Problem3CachedRunnerE14,
    config: E14Config,
    subaction_new_contact_count: int,
    raw_candidate_count: int,
) -> Dict[str, Any]:
    ...
    cell3 = problem3_runner.run_cell3(
        ...
        hand_object_iou_max=float(config.hand_object_iou_max),
        ...
        [
            {
                ...
                "clean_ref_found": int(candidate_df["ref_idx"].notna().sum()),
                "clean_ref_active_hand_visible": int((candidate_df.get("reference_mode") == "closest_clean_pre_contact_active_hand_visible").sum()) if "reference_mode" in candidate_df else 0,
                "clean_ref_humanless": int((candidate_df.get("reference_mode") == "closest_clean_pre_contact_humanless").sum()) if "reference_mode" in candidate_df else 0,
...
def save_bar_chart_failure_reasons(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E14 failure reasons")
...
def save_funnel_chart(overview_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("count")
    ax.set_title("E14 pipeline funnel")
...
def save_timeline_chart(candidate_df: pd.DataFrame, subactions: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("Timeline: E14 first-contact samples and reference arrows")
...
def save_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E14 success ref_gap distribution")
...
def save_candidate_offset_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E14 success candidate offset distribution")
...
def save_gap_length_histogram(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E14 pre-contact gap length distribution")
...
def save_reference_mode_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    fig, ax = plt.subplots(figsize=(8, 4))
    for mode, color in [
        ("closest_clean_pre_contact_active_hand_visible", "#188038"),
        ("closest_clean_pre_contact_humanless", "#1a73e8"),
    ]:
    ...
    ax.set_title("E14 success ref_gap by reference mode")
    ax.set_xlabel("candidate frame - reference frame")
    ax.set_ylabel("count")
    ax.legend()
    plt.tight_layout()
...
def write_comparison_files(candidate_df: pd.DataFrame, output_root: Path) -> None:
    ...
    for baseline_exp, baseline_root_name, out_name in [
        ("E6b", "数据处理小批量测试前100个subaction_E6b_pre_contact_gap_crop_fallback", "e14_success_vs_e6b.csv"),
        ("E6c", "数据处理小批量测试前100个subaction_E6c_object_consistency", "e14_success_vs_e6c.csv"),
    ]:
...
def write_markdown_report(
    *,
    ...
    visible_success_df = success_df[success_df["reference_mode"] == "closest_clean_pre_contact_active_hand_visible"].copy()
    humanless_success_df = success_df[success_df["reference_mode"] == "closest_clean_pre_contact_humanless"].copy()
    ...
    lines = [
        ...
        f"- clean crop gate: `{config.crop_size}x{config.crop_size}`, hand overlap <= `{config.crop_hand_overlap_max}`",
        f"- hand-object IoU max: `{config.hand_object_iou_max}`",
        f"- hand-object center distance min: `{config.min_hand_object_center_distance_px}`",
        ...
        "## 核心回答",
        "",
        f"- active-hand-visible success count: `{len(visible_success_df)}`",
        f"- humanless success count: `{len(humanless_success_df)}`",
        ...
        f"- median strict shadow anchor gap: `{distribution_stats(candidate_df['strict_reference_anchor_gap_shadow']).get('median') if 'strict_reference_anchor_gap_shadow' in candidate_df else None}`",
        f"- median anchor gap delta vs strict: `{distribution_stats(success_df['reference_anchor_gap_delta_vs_strict']).get('median') if 'reference_anchor_gap_delta_vs_strict' in success_df else None}`",
        ...
        "## success 分布",
        "",
        f"- visible-mode ref_gap stats: `{distribution_stats(visible_success_df['ref_gap']) if 'ref_gap' in visible_success_df else distribution_stats(pd.Series(dtype=float))}`",
        f"- humanless-mode ref_gap stats: `{distribution_stats(humanless_success_df['ref_gap']) if 'ref_gap' in humanless_success_df else distribution_stats(pd.Series(dtype=float))}`",
        ...
        f"- strict shadow anchor gap stats: `{distribution_stats(candidate_df['strict_reference_anchor_gap_shadow']) if 'strict_reference_anchor_gap_shadow' in candidate_df else distribution_stats(pd.Series(dtype=float))}`",
...
def save_reference_anchor_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E14 success reference_anchor_gap distribution")
...
def run_e14_experiment(config: Optional[E14Config] = None) -> Dict[str, Any]:
    ...
    for idx, row in subactions.iterrows():
        ...
        print(
            f"E14 subaction {idx:02d} {row['narration_id']} | {row['narration']} | "
    ...
    save_funnel_chart(overview_df, charts_dir / "e14_pipeline_funnel.png")
    save_bar_chart_failure_reasons(candidate_df, charts_dir / "e14_failure_reasons.png")
    save_timeline_chart(candidate_df, subactions, charts_dir / "e14_timeline.png")
    success_df = candidate_df[candidate_df["status"] == "keep"].copy()
    save_ref_gap_histogram(success_df, charts_dir / "e14_ref_gap_success_hist.png")
    save_reference_anchor_gap_histogram(success_df, charts_dir / "e14_reference_anchor_gap_hist.png")
    save_candidate_offset_histogram(success_df, charts_dir / "e14_candidate_offset_success_hist.png")
    save_gap_length_histogram(candidate_df, charts_dir / "e14_gap_length_hist.png")
    save_reference_mode_ref_gap_histogram(success_df, charts_dir / "e14_ref_gap_success_by_reference_mode_hist.png")
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e14_success_contact_sheet",
        "E14 success samples",
        per_page=30,
    )
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e14_success_heatmap_trajectory_contact_sheet",
        "E14 success heatmap + trajectory overlays",
        per_page=30,
    )
