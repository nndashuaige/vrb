"""E10 experiment: contact-part-aware point tracking and projection.

This script keeps E10's candidate / reference-frame search structure, but
replaces whole-object tracking-point sampling and mask gating with a local
contact-part mask, contact-part dominated point tracking, local affine
projection, and a part-aware gate.
"""
...
EXPERIMENT = {
    "experiment_id": "E10",
    "name": "E10_cotracker_part_aware_contact_projection",
    "description": (
        "使用 contact-part mask 把物体跟踪点限制到接触部件附近，"
        "优先用 CoTracker 跟踪这些局部点，再用局部 affine 把 contact GMM 投影回参考帧，"
        "并用部件感知的 reference mask gate 进行最终筛选。"
    ),
}
...
class E10Config:
    ...
    sam2_enabled: bool = os.environ.get("E10_SAM2_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
    sam2_model_id: str = os.environ.get("E10_SAM2_MODEL_ID", "facebook/sam2-hiera-tiny")
    sam2_device: str = os.environ.get("E10_SAM2_DEVICE", "cuda")
    sam2_multimask_output: bool = os.environ.get("E10_SAM2_MULTIMASK", "0").strip().lower() in {"1", "true", "yes", "on"}
    use_visor_mask: bool = os.environ.get("E10_USE_VISOR_MASK", "1").strip().lower() not in {"0", "false", "no", "off"}
    cotracker_enabled: bool = os.environ.get("E10_COTRACKER_ENABLED", "1").strip().lower() not in {"0", "false", "no", "off"}
    cotracker_repo: str = os.environ.get("E10_COTRACKER_REPO", "facebookresearch/co-tracker")
    cotracker_model: str = os.environ.get("E10_COTRACKER_MODEL", "cotracker3_online")
    cotracker_max_frames: int = int(os.environ.get("E10_COTRACKER_MAX_FRAMES", "16"))
    cotracker_max_side_px: int = int(os.environ.get("E10_COTRACKER_MAX_SIDE_PX", "384"))
    min_tracking_points: int = int(os.environ.get("E10_MIN_TRACKING_POINTS", "20"))
    max_tracking_points: int = int(os.environ.get("E10_MAX_TRACKING_POINTS", "36"))
    hand_exclusion_pad_px: int = int(os.environ.get("E10_HAND_EXCLUSION_PAD_PX", "6"))
    point_boundary_band_px: int = int(os.environ.get("E10_POINT_BOUNDARY_BAND_PX", "10"))
    point_contact_band_px: int = int(os.environ.get("E10_POINT_CONTACT_BAND_PX", "28"))
    part_boundary_band_px: int = int(os.environ.get("E10_PART_BOUNDARY_BAND_PX", "14"))
    part_contact_dilate_px: int = int(os.environ.get("E10_PART_CONTACT_DILATE_PX", "24"))
    part_hand_dilate_px: int = int(os.environ.get("E10_PART_HAND_DILATE_PX", "18"))
    part_edge_strip_px: int = int(os.environ.get("E10_PART_EDGE_STRIP_PX", "18"))
    part_min_tracking_points: int = int(os.environ.get("E10_PART_MIN_TRACKING_POINTS", "14"))
    part_gate_min_inside_count: int = int(os.environ.get("E10_PART_GATE_MIN_INSIDE_COUNT", "3"))
    part_gate_max_distance_px: float = float(os.environ.get("E10_PART_GATE_MAX_DISTANCE_PX", "20.0"))
    local_projection_max_median_residual: float = float(os.environ.get("E10_LOCAL_PROJECTION_MAX_MEDIAN_RESIDUAL", "10.0"))
    local_projection_min_inlier_ratio: float = float(os.environ.get("E10_LOCAL_PROJECTION_MIN_INLIER_RATIO", "0.4"))
    track_fb_max: float = float(os.environ.get("E10_TRACK_FB_MAX", "3.0"))
    track_min_visible_points: int = int(os.environ.get("E10_TRACK_MIN_VISIBLE_POINTS", "8"))
    affine_ransac_reproj_threshold: float = float(os.environ.get("E10_AFFINE_RANSAC_REPROJ_THRESHOLD", "4.0"))
    affine_min_inliers: int = int(os.environ.get("E10_AFFINE_MIN_INLIERS", "6"))
    affine_min_inlier_ratio: float = float(os.environ.get("E10_AFFINE_MIN_INLIER_RATIO", "0.35"))
    affine_max_median_residual: float = float(os.environ.get("E10_AFFINE_MAX_MEDIAN_RESIDUAL", "12.0"))
    mask_gate_min_inside_count: int = int(os.environ.get("E10_MASK_GATE_MIN_INSIDE_COUNT", "3"))
    mask_gate_boundary_tolerance_px: float = float(os.environ.get("E10_MASK_GATE_BOUNDARY_TOLERANCE_PX", "14.0"))
    mask_gate_centroid_max_distance_px: float = float(os.environ.get("E10_MASK_GATE_CENTROID_MAX_DISTANCE_PX", "18.0"))
    mask_gate_min_visible_points: int = int(os.environ.get("E10_MASK_GATE_MIN_VISIBLE_POINTS", "8"))
    mask_gate_min_inlier_ratio: float = float(os.environ.get("E10_MASK_GATE_MIN_INLIER_RATIO", "0.35"))
    final_quality_keep_threshold: float = float(os.environ.get("E10_FINAL_QUALITY_KEEP_THRESHOLD", "0.34"))
    ...
    output_root: Path = VRBREPRODUCTION_ROOT / "outputs" / "100subaction-e10"
    annotation_csv: Path = (
...
def get_subactions(config: E10Config) -> pd.DataFrame:
...
class E10FrameCache:
    def __init__(self, image_dir: Path, config: Optional[E10Config] = None):
    ...
    def load_gray_torch(self, frame_idx: int, config: E10Config):
...
def _ensure_sam2_predictor(config: E10Config):
...
def _ensure_cotracker_model(config: E10Config):
...
def _load_visor_mask(frame_idx: int, image_bgr: np.ndarray, active_object_bbox: Sequence[float], config: E10Config) -> Dict[str, Any]:
    h, w = image_bgr.shape[:2]
    candidates: List[Path] = []
    visor_root = os.environ.get("E10_VISOR_MASK_ROOT")
    if visor_root:
...
def get_object_mask(frame_idx: int, image_bgr: np.ndarray, active_object_bbox: Sequence[float], noun: Any, config: E10Config) -> Dict[str, Any]:
    h, w = image_bgr.shape[:2]
...
def track_object_points_to_reference(
    ...
    if model is not None:
        try:
            ...
            print(f"E10 CoTracker runtime fallback: {type(exc).__name__}:{exc}", flush=True)
            backend = "fallback_lk"
...
def get_torch_for_gpu_flow(config: E10Config):
...
def describe_gpu_flow_backend(config: E10Config) -> str:
    torch = get_torch_for_gpu_flow(config)
    if torch is None:
        if not config.gpu_flow_enabled:
...
def reserve_gpu_workspace(config: E10Config) -> None:
...
def preload_gpu_gray_cache(cache: E10FrameCache, config: E10Config) -> None:
...
def track_points_backward_torch_patch(
    ...
    pts_cur: np.ndarray,
    config: E10Config,
    cache: Optional[E10FrameCache] = None,
...
def track_points_backward(
    ...
    prev_gray: np.ndarray,
    pts_cur: np.ndarray,
    config: Optional[E10Config] = None,
    cache: Optional[E10FrameCache] = None,
    cur_frame_idx: Optional[int] = None,
    prev_frame_idx: Optional[int] = None,
    fb_max: float = 2.5,
...
def save_sample_outputs(
    *,
    ...
    heatmap_mode = "covariance_e10_object_aware" if covariances is not None else "isotropic_e7"
...
def finalize_one_sample_per_subaction(candidate_df: pd.DataFrame, config: Optional[E10Config] = None) -> pd.DataFrame:
...
def save_funnel_chart(overview_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("count")
    ax.set_title("E10 pipeline funnel")
...
def save_failure_chart(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E10 failure reasons")
...
def save_track_confidence_hist(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E10 track confidence")
...
def save_projection_source_counts(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E10 projection source counts")
...
def save_success_by_subaction(subaction_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E10 success by subaction")
...
def write_summary(
    *,
    ...
    compare_lines.append(
        f"| E10 | {int(row['subactions_success'])} | {int(row['final_tuple_count'])} | "
    ...
    lines = [
        ...
        "",
        f"E10 当前覆盖 `{int(row['subactions_success'])}/100` 个 subaction（子动作片段），距离 82 个 subaction 目标还差 `{max(0, 82 - int(row['subactions_success']))}` 个。`final_tuple_count` 现在反映 top-k 导出口径，不再等同于 subaction 覆盖数。",
        "",
        "## 质量风险",
        "",
...
def run_e10_experiment(config: Optional[E10Config] = None) -> Dict[str, Any]:
    ...
    cache = E10FrameCache(config.image_dir, config=config)
    ...
    if not _GPU_FLOW_STATUS_PRINTED:
        print(f"E10 optical-flow backend: {describe_gpu_flow_backend(config)}", flush=True)
    ...
    for progress_idx, (idx, row) in enumerate(subactions.iterrows(), start=1):
        ...
        print(
            ...
            f"E10 subaction {idx:02d} {row['narration_id']} | {row['narration']} | "
    ...
    save_funnel_chart(overview_df, charts_dir / "e10_pipeline_funnel.png")
    save_failure_chart(candidate_df, charts_dir / "e10_failure_reasons.png")
    save_track_confidence_hist(candidate_df, charts_dir / "e10_track_confidence_hist.png")
    save_projection_source_counts(candidate_df, charts_dir / "e10_projection_source_counts.png")
    save_success_by_subaction(subaction_df, charts_dir / "e10_success_by_subaction.png")
    make_contact_sheet(final_df, charts_dir / "e10_mask_projection_contact_sheet.png", "E10 mask projection samples", page_size=1000)
    contact_sheet_pages = make_contact_sheet(final_df, charts_dir / "e10_success_contact_sheet_page_001.png", "E10 final success samples", page_size=16)
