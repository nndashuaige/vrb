"""E18b experiment: paper-faithful variant of E18 (no self-added gates) + Cell2 neighbor-frame retry.

与 E18 的差异（其余逐字节一致，保证单变量可比）：
1. Cell2 邻帧重试：t0 失败则在 t0+1..t0+K (K=cell2_retry_max_offset) 内重试，仅尝试 active hand
   smoothed contact binary==1 的帧（跳过闪断的 0 帧，相当于隐式 run 去抖）。
2. 移除 Cell2 最少 5 点 gate：GMM n_components=min(5, 点数)，仅 0 点视为失败（无可拟合数据）。
3. 移除 homography 质量 gate（good_matches>=60 / inliers>=40 / inlier_ratio>=0.45）；
   仅当 H 计算不出（特征/匹配不足 10、RANSAC 失败）才失败。质量数值仍记录供 shadow 分析。
4. 移除 geometry gate（projected_area/area_ratio、inside_count、centroid_dist、
   no_active_object_bbox）——全部降级为 record-only shadow 字段。
5. 保留论文明确叙述的 discard：投影 contact points 或 trajectory 出画
   ("If the contact points or trajectory are not in the frame ... we then discard")。
"""
...
from .pipeline_retention import _default_contact_config, _json_safe, run_cell4_heatmap_gate
...
class E18BConfig:
    ...
    output_root: Path = VRBREPRODUCTION_ROOT / "outputs" / "e18b"
...
EXPERIMENT = {
    "experiment_id": "E18B",
    "name": "E18b_paper_faithful_cell2_retry",
    "contact_strategy": "first_new_contact_episode_with_cell2_neighbor_retry",
    ...
    "description": (
        "E18 的论文忠实版：episode/reference 逻辑与 E18 一致；"
        "Cell2 在 t0 失败时于 t0+1..t0+cell2_retry_max_offset 内重试（仅 contact binary==1 的帧）；"
        "移除自加 gate：Cell2 最少点数 gate（GMM n_components=min(5,点数)）、"
        "homography 质量三阈值（good_matches>=60/inliers>=40/ratio>=0.45）、"
        "geometry gate（area_ratio/inside_count/centroid_dist）——全部降级为 record-only shadow 字段；"
        "仅保留论文明确的 discard：投影 contact points / trajectory 出画。"
    ),
}
...
def fit_cell2_contact_gmm_paper_faithful(
    detections,
    sample: Dict[str, Any],
    image_dir: Path,
    ...
    """E18B Cell2：提取流程与 pipeline_retention.fit_cell2_contact_gmm 完全一致，
    但移除自加的最少 5 点 gate：n_components = min(5, 点数)，仅 0 点视为失败。"""
    ...
    if img is None:
        return {"passed": False, "reason": "missing_frame", "contact_points": 0, "gmm_n_components_used": 0}
    ...
    if active_hand_bbox is None or len(object_bboxes) == 0:
        return {"passed": False, "reason": "missing_hand_or_object_bbox", "contact_points": 0, "gmm_n_components_used": 0}

    extraction = extract_contact_points(img, active_hand_bbox, object_bboxes, config=config)
    contact_points = extraction["contact_points"]
    contact_count = int(len(contact_points))
    if contact_count < 1:
        return {
            "passed": False,
            "reason": "no_contact_points_after_object_boundary_filter",
            "contact_points": 0,
            "gmm_n_components_used": 0,
        }

    n_components = int(min(int(max_components), contact_count))
    try:
        ...
        return {
            ...
            "gmm_n_components_used": n_components,
        }
...
def get_subactions(config: E18BConfig) -> pd.DataFrame:
...
class Problem3CachedRunnerE18B:
    def __init__(
        self,
        detections,
        image_dir: Path,
    ...
    def accumulate_homography_to_ref(self, ref_idx: int, target_idx: int):
        ...
        for cur_idx in range(target_idx, ref_idx, -1):
            ...
            # E18B: 论文忠实版不设 homography 质量 gate；仅记录 shadow 判定，不 discard。
            stats["passed_quality_gate"] = not (
                stats["good_matches"] < 60 or stats["inliers"] < 40 or stats["inlier_ratio"] < 0.45
            )
    ...
    def run_cell3(
        ...
        *,
        ...
        result: Dict[str, Any] = {
            ...
            "inside_count": None,
            "centroid_dist": None,
            "shadow_geometry_pass": None,
            "shadow_geometry_fail_reasons": None,
        ...
        # E18B: geometry gate 全部降级为 record-only shadow 字段，不再 discard。
        # 论文唯一保留的 discard 是投影点/轨迹出画（见下方 out_of_bounds 检查）。
        object_polygon_ref = None
        shadow_geometry_fail_reasons: List[str] = []
        if active_object_bbox_norm is None:
            shadow_geometry_fail_reasons.append("no_active_object_bbox")
        else:
            ...
            if projected_area <= 0 or area_ratio < 0.15:
                shadow_geometry_fail_reasons.append("projected_object_polygon_invalid")
            ...
            if inside_count < 4:
                shadow_geometry_fail_reasons.append("transformed_contact_points_off_object")
            ...
            if centroid_dist < -8.0:
                shadow_geometry_fail_reasons.append("transformed_contact_centroid_off_object")
        result["shadow_geometry_fail_reasons"] = shadow_geometry_fail_reasons
        result["shadow_geometry_pass"] = bool(len(shadow_geometry_fail_reasons) == 0)
        ...
        overlay_object_polygon = (
            object_polygon_ref
            if object_polygon_ref is not None
...
def base_candidate_record(
    *,
    ...
    return {
        ...
        "fail_reason": None,
        "contact_points": 0,
        "cell2_retry_window_end_0_based": None,
        "cell2_retry_attempts": 0,
        "cell2_retry_offset_used": None,
        "cell2_retry_rescued": False,
        "cell2_t0_fail_reason": None,
        "cell2_attempt_log": None,
        "gmm_n_components_used": None,
        "shadow_geometry_pass": None,
        "shadow_geometry_fail_reasons": None,
        "ref_idx": None,
...
def run_one_candidate(
    *,
    ...
    problem3_runner: Problem3CachedRunnerE18B,
    config: E18BConfig,
    subaction_new_contact_count: int,
    raw_candidate_count: int,
) -> Dict[str, Any]:
    ...
    # --- E18B Cell2 邻帧重试 ---
    # 窗口上限 = min(t0 + cell2_retry_max_offset, episode run 右边界, subaction 右边界)。
    # run 内帧全为 smoothed contact binary==1（run 定义即连续 1 段），
    # 因此"信号跌回 0 即停"由 run 右边界自然保证。
    t0 = int(candidate["frame"])
    episode_end = int(candidate["episode_contact_end_frame_0_based"])
    retry_window_end = min(
        t0 + int(config.cell2_retry_max_offset),
        episode_end,
        stop_0,
        len(detections) - 1,
    )
    ...
    cell2_frame = t0
    cell2_attempts = []
    for attempt_frame in range(t0, retry_window_end + 1):
        sample = {
        ...
        attempt = fit_cell2_contact_gmm_paper_faithful(
            detections=detections,
            sample=sample,
            image_dir=config.image_dir,
            contact_extraction_config=contact_config,
        )
        cell2_attempts.append(
            {
                "frame_0_based": int(attempt_frame),
                "offset_from_t0": int(attempt_frame - t0),
                "passed": bool(attempt["passed"]),
                "reason": attempt.get("reason"),
                "contact_points": int(attempt.get("contact_points", 0)),
            }
        )
        if attempt["passed"]:
            cell2 = attempt
            cell2_frame = int(attempt_frame)
            break
    if cell2 is None:
        cell2 = attempt  # 最后一次失败结果（记录其 reason）

    record["cell2_retry_window_end_0_based"] = int(retry_window_end)
    record["cell2_retry_attempts"] = int(len(cell2_attempts))
    record["cell2_retry_offset_used"] = int(cell2_frame - t0) if cell2["passed"] else None
    record["cell2_retry_rescued"] = bool(cell2["passed"] and cell2_frame != t0)
    record["cell2_t0_fail_reason"] = cell2_attempts[0]["reason"] if not cell2_attempts[0]["passed"] else None
    record["cell2_attempt_log"] = cell2_attempts
    record["gmm_n_components_used"] = int(cell2.get("gmm_n_components_used", 0))

    # 实际用于 Cell2/Cell3 的 contact frame（可能是 t0+k）
    record["frame_0_based"] = int(cell2_frame)
    record["frame_1_based"] = int(cell2_frame) + 1
    record["candidate_frame_offset_from_episode_start"] = int(cell2_frame - int(candidate["episode_contact_start_frame_0_based"]))
    record["is_candidate_at_episode_start"] = bool(record["candidate_frame_offset_from_episode_start"] == 0)
    record["is_candidate_near_episode_start"] = bool(record["candidate_frame_offset_from_episode_start"] <= 2)
    ...
    sample_dir = (
        ...
        / f"frame_{int(cell2_frame):06d}_{candidate['hand']}"
    )
    ...
    for field in [
        ...
        "shadow_geometry_pass",
        "shadow_geometry_fail_reasons",
    ]:
        ...
        [
            {
                ...
                "cell2_retry_rescued": int(candidate_df.get("cell2_retry_rescued", pd.Series(dtype=bool)).fillna(False).astype(bool).sum()),
                "cell2_retry_rescued_keep": int(((candidate_df["status"] == "keep") & candidate_df.get("cell2_retry_rescued", pd.Series(dtype=bool)).fillna(False).astype(bool)).sum()),
                "shadow_geometry_fail_keep": int(((candidate_df["status"] == "keep") & (candidate_df.get("shadow_geometry_pass") == False)).sum()) if "shadow_geometry_pass" in candidate_df else 0,
                "shadow_homography_low_quality_keep": int(((candidate_df["status"] == "keep") & (candidate_df.get("homography_min_inliers", pd.Series(dtype=float)).fillna(9999) < 40)).sum()) if "homography_min_inliers" in candidate_df else 0,
...
def save_bar_chart_failure_reasons(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18B failure reasons")
...
def save_funnel_chart(overview_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("count")
    ax.set_title("E18B pipeline funnel")
...
def save_timeline_chart(candidate_df: pd.DataFrame, subactions: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("Timeline: E18B first-contact samples and reference arrows")
...
def save_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18B success ref_gap distribution")
...
def save_candidate_offset_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18B success candidate offset distribution")
...
def save_gap_length_histogram(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18B pre-contact gap length distribution")
...
def save_reference_mode_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18B success ref_gap by reference mode")
    ax.set_xlabel("candidate frame - reference frame")
    ax.set_ylabel("count")
    ax.legend()
    plt.tight_layout()
...
def write_markdown_report(
    *,
    ...
    lines = [
        "# E18B paper-faithful（去自加 gate）+ Cell2 邻帧重试 - 诊断实验报告",
        "",
...
def run_e18b_experiment(config: Optional[E18BConfig] = None) -> Dict[str, Any]:
    config = config or E18BConfig()
    ...
    for idx, row in subactions.iterrows():
        ...
        print(
            f"E18B subaction {idx:02d} {row['narration_id']} | {row['narration']} | "
    ...
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e18b_success_contact_sheet",
        "E18B success samples",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e18b_success_contact_sheet_page_001.png",
        "E18B success samples",
    )
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e18b_success_heatmap_trajectory_contact_sheet",
        "E18B success heatmap + trajectory overlays",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e18b_success_heatmap_trajectory_contact_sheet_page_001.png",
        "E18B success heatmap + trajectory overlays",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run E18b paper-faithful (gate-free) variant with Cell2 neighbor-frame retry.")
    parser.add_argument("--video-id", default=E18BConfig.video_id)
    parser.add_argument("--num-subactions", type=int, default=E18BConfig.num_subactions)
    parser.add_argument("--max-ref-backtrack", type=int, default=E18BConfig.max_ref_backtrack)
    parser.add_argument("--crop-size", type=int, default=E18BConfig.crop_size)
    parser.add_argument("--cell2-retry-max-offset", type=int, default=E18BConfig.cell2_retry_max_offset)
    parser.add_argument("--output-root", type=Path, default=E18BConfig.output_root)
...
if __name__ == "__main__":
    ...
    run_e18b_experiment(
        E18BConfig(
            ...
            cell2_retry_max_offset=args.cell2_retry_max_offset,
