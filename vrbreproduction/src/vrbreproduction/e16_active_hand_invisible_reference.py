"""E16 experiment: active-hand-invisible reference."""
...
class E16Config:
    ...
    output_root: Path = VRBREPRODUCTION_ROOT / "outputs" / "e16"
...
EXPERIMENT = {
    "experiment_id": "E16",
    "name": "E16_active_hand_invisible_reference",
    "contact_strategy": "first_new_contact_episode_first_timestep",
    "reference_strategy": "paper_faithful_active_hand_invisible_reference",
    "reference_fallback": "none",
    "description": (
        "每个 subaction 只取第一个 smoothed contact 0->1 episode 的 first contact timestep；"
        "reference 使用 contact episode start 之前最近的 active-hand-invisible frame；"
        "strict global no-hand 只作为 shadow diagnostics，不使用 E14 clean hand-object/crop gate。"
    ),
}
...
def get_subactions(config: E16Config) -> pd.DataFrame:
...
class Problem3CachedRunnerE16:
    ...
    def find_paper_faithful_active_hand_invisible_reference(
        self,
        *,
        t_contact: int,
        active_hand: str,
        ...
        if selected_ref_idx is None:
            detail["reference_reject_reasons"] = {"no_active_hand_invisible_reference": 1}
            return None, detail
        ...
        detail.update(
            ...
            reference_reason="closest_pre_contact_active_hand_invisible",
    ...
    def run_cell3(
        ...
        *,
        ...
        if search_end < search_start:
            result.update(
                discard=True,
                discard_reason="no_active_hand_invisible_reference",
                status="DISCARD",
                no_pre_episode_reference_window_not_rescued=True,
                reference_mode="no_active_hand_invisible_reference",
                reference_reason="no_active_hand_invisible_reference",
            )
            return result
        ...
        ref_idx, detail = self.find_paper_faithful_active_hand_invisible_reference(
...
def run_one_candidate(
    *,
    ...
    problem3_runner: Problem3CachedRunnerE16,
    config: E16Config,
    subaction_new_contact_count: int,
    raw_candidate_count: int,
) -> Dict[str, Any]:
    ...
    if cell3.get("discard") or cell3.get("status") != "KEEP":
        ...
        failed_stage = "reference_selection" if reason == "no_active_hand_invisible_reference" else "Cell3_homography_geometry"
        ...
        [
            {
                ...
                "active_hand_invisible_ref_found": int(candidate_df["ref_idx"].notna().sum()),
...
def save_bar_chart_failure_reasons(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E16 failure reasons")
...
def save_funnel_chart(overview_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("count")
    ax.set_title("E16 pipeline funnel")
...
def save_timeline_chart(candidate_df: pd.DataFrame, subactions: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("Timeline: E16 first-contact samples and reference arrows")
...
def save_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E16 success ref_gap distribution")
...
def save_candidate_offset_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E16 success candidate offset distribution")
...
def save_gap_length_histogram(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E16 pre-contact gap length distribution")
...
def save_reference_mode_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E16 success ref_gap by reference mode")
    ax.set_xlabel("candidate frame - reference frame")
    ax.set_ylabel("count")
    ax.legend()
    plt.tight_layout()
...
def write_comparison_files(candidate_df: pd.DataFrame, output_root: Path) -> None:
    ...
    for baseline_exp, baseline_root_name, out_name in [
        ("E6b", "数据处理小批量测试前100个subaction_E6b_pre_contact_gap_crop_fallback", "e16_success_vs_e6b.csv"),
        ("E6c", "数据处理小批量测试前100个subaction_E6c_object_consistency", "e16_success_vs_e6c.csv"),
    ]:
...
def write_markdown_report(
    *,
    ...
    lines = [
        "# E16 active-hand-invisible reference baseline - 诊断实验报告",
        "",
        ...
        "- reference gate: closest `active_hand_invisible_pre_contact`; strict global no-hand is recorded as shadow only.",
        ...
        "- reference 必须在 episode start 之前，并选择最近的 active hand 不可见帧。",
        "- 找不到 active-hand-invisible reference 时主结果直接失败，不用 E14 clean reference 救回。",
        "- E16 用来检验论文语义是否更接近 first-human-entry / active-hand-invisible reference，而不是 E15 的 strict global no-hand。",
        "",
        "## Funnel",
        "",
        markdown_table(
        ...
        "",
        "## E16 vs E15 vs E14 vs E6b vs E6c 对比",
        "",
        "E6b/E6c 是 multi-candidate per subaction，candidate-level heatmap_pass 不能直接和 E16 的 candidate count 对齐；`subactions_success` 是更公平口径。E14/E15/E16 都是 one candidate per subaction，更可比。",
        "",
        markdown_table(
            compare_overview,
            [
        ...
        "## 图表",
        "",
        "- pipeline funnel: `charts/e16_pipeline_funnel.png`",
        "- failure reasons: `charts/e16_failure_reasons.png`",
        "- success ref_gap histogram: `charts/e16_ref_gap_success_hist.png`",
        "- success reference_anchor_gap histogram: `charts/e16_reference_anchor_gap_hist.png`",
        "- success contact sheet: `charts/e16_success_contact_sheet_page_001.png`",
        "- success heatmap+trajectory contact sheet: `charts/e16_success_heatmap_trajectory_contact_sheet_page_001.png`",
        "",
        "## 明细文件",
        "",
        ...
        "- E16 vs E6b success table: `e16_success_vs_e6b.csv`",
        "- E16 vs E6c success table: `e16_success_vs_e6c.csv`",
        "- pipeline outputs: `experiments/E16/pipeline_outputs/`",
    ]
...
def save_reference_anchor_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E16 success reference_anchor_gap distribution")
...
def run_e16_experiment(config: Optional[E16Config] = None) -> Dict[str, Any]:
    ...
    for idx, row in subactions.iterrows():
        ...
        print(
            f"E16 subaction {idx:02d} {row['narration_id']} | {row['narration']} | "
    ...
    save_funnel_chart(overview_df, charts_dir / "e16_pipeline_funnel.png")
    save_bar_chart_failure_reasons(candidate_df, charts_dir / "e16_failure_reasons.png")
    save_timeline_chart(candidate_df, subactions, charts_dir / "e16_timeline.png")
    success_df = candidate_df[candidate_df["status"] == "keep"].copy()
    save_ref_gap_histogram(success_df, charts_dir / "e16_ref_gap_success_hist.png")
    save_reference_anchor_gap_histogram(success_df, charts_dir / "e16_reference_anchor_gap_hist.png")
    save_candidate_offset_histogram(success_df, charts_dir / "e16_candidate_offset_success_hist.png")
    save_gap_length_histogram(candidate_df, charts_dir / "e16_gap_length_hist.png")
    save_reference_mode_ref_gap_histogram(success_df, charts_dir / "e16_ref_gap_success_by_reference_mode_hist.png")
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e16_success_contact_sheet",
        "E16 success samples",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e16_success_contact_sheet_page_001.png",
        "E16 success samples",
    )
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e16_success_heatmap_trajectory_contact_sheet",
        "E16 success heatmap + trajectory overlays",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e16_success_heatmap_trajectory_contact_sheet_page_001.png",
        "E16 success heatmap + trajectory overlays",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run E16 active-hand-invisible reference baseline.")
    parser.add_argument("--video-id", default=E16Config.video_id)
    parser.add_argument("--num-subactions", type=int, default=E16Config.num_subactions)
    parser.add_argument("--max-ref-backtrack", type=int, default=E16Config.max_ref_backtrack)
    parser.add_argument("--crop-size", type=int, default=E16Config.crop_size)
    parser.add_argument("--output-root", type=Path, default=E16Config.output_root)
