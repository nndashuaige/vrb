"""E18-a experiment: use new-contact episodes as the training sequence unit."""
...
class E18AConfig:
    ...
    episode_scope: str = "continuous_first_100_subactions"
    ...
    output_root: Path = VRBREPRODUCTION_ROOT / "outputs" / "e18-a"
...
EXPERIMENT = {
    "experiment_id": "E18-a",
    "name": "E18a_new_contact_episode_unit_hybrid_min_iou_reference",
    "contact_strategy": "new_contact_episode_unit_first_timestep",
    "reference_strategy": "subaction_bounded_active_hand_invisible_then_min_hand_object_iou",
    "reference_fallback": "min_hand_object_iou_pre_contact",
    "description": (
        "把前 100 个 subaction 覆盖的连续时间范围内每个 smoothed contact 0->1 run 作为一个 new-contact episode；"
        "每个 episode 只取 first contact timestep；reference 在 episode start 之前搜索，最多向前回看 max_ref_backtrack 帧；"
        "先使用最近的 active-hand-invisible frame，找不到时 fallback 到 hand-object IoU 最小帧。"
    ),
}
...
def get_subactions(config: E18AConfig) -> pd.DataFrame:
...
def _source_subaction_for_episode(subactions: pd.DataFrame, run_start: int) -> Tuple[pd.Series, str]:
    containing = subactions[
        (subactions["start_frame"].astype(int) - 1 <= int(run_start))
        & (subactions["stop_frame"].astype(int) - 1 >= int(run_start))
    ]
    if not containing.empty:
        return containing.iloc[0], "contains_episode_start"
    distances = np.minimum(
        np.abs(subactions["start_frame"].astype(int).to_numpy() - 1 - int(run_start)),
        np.abs(subactions["stop_frame"].astype(int).to_numpy() - 1 - int(run_start)),
    )
    nearest_pos = int(np.argmin(distances))
    return subactions.iloc[nearest_pos], "nearest_temporal_subaction"


def build_episode_rows_from_new_contact_runs(
    *,
    subactions: pd.DataFrame,
    all_runs: Sequence[Dict[str, Any]],
    left_binary: np.ndarray,
    right_binary: np.ndarray,
    config: E18AConfig,
) -> Tuple[pd.DataFrame, Dict[str, int]]:
    source_start_0 = max(0, int(subactions["start_frame"].min()) - 1)
    source_stop_0 = int(subactions["stop_frame"].max()) - 1
    rows: List[Dict[str, Any]] = []
    for episode_index, run in enumerate(
        [
            item
            for item in all_runs
            if source_start_0 <= int(item["start_frame_0_based"]) <= source_stop_0
        ]
    ):
        run_start = int(run["start_frame_0_based"])
        run_end = int(run["end_frame_0_based"])
        source_row, assignment = _source_subaction_for_episode(subactions, run_start)
        source_index = int(source_row.name)
        rows.append(
            {
                "video_id": source_row["video_id"],
                "narration_id": f"{config.video_id}_newcontact_{episode_index:04d}",
                "narration": f"new contact episode {episode_index:04d} ({run['hand']}) from {source_row['narration']}",
                "verb": source_row["verb"],
                "noun": source_row["noun"],
                "all_nouns": source_row.get("all_nouns"),
                "start_frame": run_start + 1,
                "stop_frame": run_end + 1,
                "episode_index": int(episode_index),
                "episode_hand": run["hand"],
                "source_subaction_index": source_index,
                "source_narration_id": source_row["narration_id"],
                "source_narration": source_row["narration"],
                "source_start_frame": int(source_row["start_frame"]),
                "source_stop_frame": int(source_row["stop_frame"]),
                "source_assignment": assignment,
                "source_window_start_frame_1_based": source_start_0 + 1,
                "source_window_stop_frame_1_based": source_stop_0 + 1,
                "episode_run_length": int(run["run_length"]),
                "episode_start_reason": run["start_reason"],
            }
        )
    episode_rows = pd.DataFrame(rows)
    if not episode_rows.empty:
        episode_rows = episode_rows.reset_index(drop=True)
    frame_coverage = compute_subaction_frame_coverage(subactions)
    frame_coverage.update(
        {
            "episode_scope_start_frame_1_based": int(source_start_0 + 1),
            "episode_scope_stop_frame_1_based": int(source_stop_0 + 1),
            "new_contact_episode_rows": int(len(episode_rows)),
            "left_episode_rows": int((episode_rows["episode_hand"] == "left").sum()) if not episode_rows.empty else 0,
            "right_episode_rows": int((episode_rows["episode_hand"] == "right").sum()) if not episode_rows.empty else 0,
            "left_contact_frames_in_scope": int(left_binary[source_start_0 : source_stop_0 + 1].sum()),
            "right_contact_frames_in_scope": int(right_binary[source_start_0 : source_stop_0 + 1].sum()),
        }
    )
    return episode_rows, frame_coverage


def build_candidate_for_episode_row(
    *,
    row: pd.Series,
    left_binary: np.ndarray,
    right_binary: np.ndarray,
    ...
    run_start = int(row["start_frame"]) - 1
    run_end = int(row["stop_frame"]) - 1
    hand = str(row["episode_hand"])
    pre_gap_start = compute_pre_contact_gap_start(
        run_start=run_start,
        hand=hand,
        left_binary=left_binary,
    ...
    effective_start = max(0, run_start - int(max_ref_backtrack))
    effective_end = run_start - 1
    window_truncated = bool(effective_start > pre_gap_start)
    candidates: List[Dict[str, Any]] = []
    for offset in early_offsets:
        candidate_frame = run_start + int(offset)
        if candidate_frame > run_end:
            continue
        candidates.append(
            {
                "frame": int(candidate_frame),
                ...
                "episode_contact_start_frame_0_based": run_start,
                "episode_contact_end_frame_0_based": run_end,
                "episode_contact_run_length": int(row["episode_run_length"]),
                "episode_id_within_subaction": int(row.name) + 1,
                "episode_start_hand": hand,
                "episode_start_reason": row["episode_start_reason"],
                "candidate_is_episode_representative": bool(int(offset) == 0),
                "pre_contact_gap_start_frame_0_based": int(pre_gap_start),
                "pre_contact_gap_end_frame_0_based": int(pre_gap_end),
                "gap_length_before_episode_start": int(gap_length),
                "raw_semantic_window_start_0_based": int(pre_gap_start),
                "raw_semantic_window_end_0_based": int(pre_gap_end),
                "effective_reference_window_start_0_based": int(effective_start),
                "effective_reference_window_end_0_based": int(effective_end),
                "max_ref_backtrack": int(max_ref_backtrack),
                "window_truncated_by_max_backtrack": bool(window_truncated),
    ...
    return candidates, episode_meta
...
def base_candidate_record(
    *,
    ...
    return {
        ...
        "episode_unit_index": int(row.get("episode_index", row.name)),
        "source_subaction_index": int(row.get("source_subaction_index", row.name)),
        "source_narration_id": row.get("source_narration_id", row["narration_id"]),
        "source_narration": row.get("source_narration", row["narration"]),
        ...
        "episode_id": f"{row['video_id']}_newcontact_{int(row.get('episode_index', row.name)):04d}_{candidate['hand']}",
        "episode_reason": "smoothed_contact_0_to_1_episode_unit",
...
def run_one_candidate(
    *,
    ...
    problem3_runner: Problem3CachedRunnerE18,
    config: E18AConfig,
    subaction_new_contact_count: int,
    raw_candidate_count: int,
) -> Dict[str, Any]:
    ...
    sample_dir = (
        ...
        / "pipeline_outputs"
        / f"episode_{int(row.name):04d}_{row['narration_id']}"
        / f"contact_{int(candidate['episode_id_within_subaction']):04d}_{candidate['hand']}"
...
def save_bar_chart_failure_reasons(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18 failure reasons")
...
def save_funnel_chart(overview_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("count")
    ax.set_title("E18 pipeline funnel")
...
def save_timeline_chart(candidate_df: pd.DataFrame, subactions: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("Timeline: E18 first-contact samples and reference arrows")
...
def save_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18 success ref_gap distribution")
...
def save_candidate_offset_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18 success candidate offset distribution")
...
def save_gap_length_histogram(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18 pre-contact gap length distribution")
...
def save_reference_mode_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18 success ref_gap by reference mode")
    ax.set_xlabel("candidate frame - reference frame")
    ax.set_ylabel("count")
    ax.legend()
    plt.tight_layout()
...
def write_comparison_files(candidate_df: pd.DataFrame, output_root: Path) -> None:
    ...
    for baseline_exp, baseline_root_name, out_name in [
        ("E6b", "数据处理小批量测试前100个subaction_E6b_pre_contact_gap_crop_fallback", "e18_success_vs_e6b.csv"),
        ("E6c", "数据处理小批量测试前100个subaction_E6c_object_consistency", "e18_success_vs_e6c.csv"),
    ]:
...
def write_markdown_report(
    *,
    ...
    lines = [
        "# E18-a new-contact episode unit hybrid min-IoU reference - 诊断实验报告",
        "",
        f"- video_id: `{config.video_id}`",
        f"- source subactions: first `{config.num_subactions}` annotations of this video",
        f"- episode scope: `{config.episode_scope}`",
        ...
        f"- episode scope frames: `{frame_coverage.get('episode_scope_start_frame_1_based')}-{frame_coverage.get('episode_scope_stop_frame_1_based')}`",
        f"- new-contact episodes: `{frame_coverage.get('new_contact_episode_rows')}` (`left={frame_coverage.get('left_episode_rows')}`, `right={frame_coverage.get('right_episode_rows')}`)",
        "",
        "## E18-a 定义",
        "",
        "- 一个 smoothed contact `0->1` run 视作一个 new-contact episode/training sequence，最多输出一个 final tuple。",
        "- 左右手分开统计 episode；episode 纳入条件是 run 的真实 start frame 落在 source window 内。",
        "- first contact timestep 是该 episode 的 start frame，active hand 使用该 run 的 hand。",
        "- episode 的 noun/narration 继承包含该 start frame 的 source subaction；如果 start 落在 gap 中，则继承最近 subaction 并在 `source_assignment` 记录。",
        ...
        "",
        "## E18-a vs E15 vs E14 vs E6b vs E6c 对比",
        "",
        "E18-a 是 one candidate per new-contact episode；E14/E15/E18 是 one candidate per subaction；E6b/E6c 是 multi-candidate per subaction，所以这里的 `subactions_success` 在 E18-a 中实际表示 episode success。",
        "",
        markdown_table(
            compare_overview,
            [
        ...
        "- episode summary CSV: `episode_summary.csv`",
        "- compatibility summary CSV: `subaction_summary.csv`",
        ...
        "- pipeline outputs: `experiments/E18-a/pipeline_outputs/`",
    ]
...
def save_reference_anchor_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E18 success reference_anchor_gap distribution")
...
def run_e18a_experiment(config: Optional[E18AConfig] = None) -> Dict[str, Any]:
    config = config or E18AConfig()
    ...
    episode_rows, frame_coverage = build_episode_rows_from_new_contact_runs(
    ...
    print(f"Selected source subactions: {len(subactions)}")
    print(f"New-contact episode rows: {len(episode_rows)}")
    ...
    print(
        "Episode scope frames: "
        f"{frame_coverage['episode_scope_start_frame_1_based']}-"
        f"{frame_coverage['episode_scope_stop_frame_1_based']}"
    )
    ...
    for idx, row in episode_rows.iterrows():
        ...
        flattened_candidates, episode_meta = build_candidate_for_episode_row(
            row=row,
            left_binary=left_binary,
            right_binary=right_binary,
            early_offsets=config.early_candidate_offsets,
        ...
        candidate_plan_rows.append(
            {
                "experiment_id": EXPERIMENT["experiment_id"],
                "subaction_index": int(idx),
                "episode_unit_index": int(row.get("episode_index", idx)),
                "episode_hand": row["episode_hand"],
                "source_subaction_index": int(row["source_subaction_index"]),
                "source_narration_id": row["source_narration_id"],
                "source_narration": row["source_narration"],
                ...
                "episode_filtered_candidate_count": 1,
                "deep_run_candidate_count": int(len(flattened_candidates)),
                "episode_decision": episode_decision,
                "episode_reason": episode_reason,
                "subaction_new_contact_count": 1,
                "episode_start_frames_0_based": str(int(start_0)),
                "episode_hands": str(row["episode_hand"]),
                "pre_contact_gap_start_frame_0_based": episode_meta["pre_contact_gap_start_frame_0_based"],
                "pre_contact_gap_end_frame_0_based": episode_meta["pre_contact_gap_end_frame_0_based"],
                "gap_length_before_episode_start": episode_meta["gap_length_before_episode_start"],
                "reference_search_window_start_0_based": episode_meta["reference_search_window_start_0_based"],
                "reference_search_window_end_0_based": episode_meta["reference_search_window_end_0_based"],
                "window_truncated_by_max_backtrack": episode_meta["window_truncated_by_max_backtrack"],
            }
        )

        print(
            f"E18-a episode {idx:04d} {row['narration_id']} | {row['episode_hand']} | "
            f"source={row['source_narration_id']} | raw_candidates={len(raw_candidates)} | new_contact_runs=1 | "
    ...
    subaction_summary_df = summarize_subactions(candidate_df, episode_rows, candidate_plan_df)
            ...
            _json_safe(
                {
                    ...
                    "source_subactions": subactions.to_dict(orient="records"),
                    "episode_rows": episode_rows.to_dict(orient="records"),
    ...
    subaction_summary_df.to_csv(config.output_root / "episode_summary.csv", index=False)
    ...
    save_funnel_chart(overview_df, charts_dir / "e18_pipeline_funnel.png")
    save_bar_chart_failure_reasons(candidate_df, charts_dir / "e18_failure_reasons.png")
    save_timeline_chart(candidate_df, episode_rows, charts_dir / "e18a_timeline.png")
    success_df = candidate_df[candidate_df["status"] == "keep"].copy()
    save_ref_gap_histogram(success_df, charts_dir / "e18_ref_gap_success_hist.png")
    save_reference_anchor_gap_histogram(success_df, charts_dir / "e18_reference_anchor_gap_hist.png")
    save_candidate_offset_histogram(success_df, charts_dir / "e18_candidate_offset_success_hist.png")
    save_gap_length_histogram(candidate_df, charts_dir / "e18_gap_length_hist.png")
    save_reference_mode_ref_gap_histogram(success_df, charts_dir / "e18_ref_gap_success_by_reference_mode_hist.png")
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e18_success_contact_sheet",
        "E18 success samples",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e18_success_contact_sheet_page_001.png",
        "E18 success samples",
    )
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e18_success_heatmap_trajectory_contact_sheet",
        "E18 success heatmap + trajectory overlays",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e18_success_heatmap_trajectory_contact_sheet_page_001.png",
        "E18 success heatmap + trajectory overlays",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run E18-a hybrid active-hand-invisible plus min-IoU fallback reference baseline.")
    parser.add_argument("--video-id", default=E18AConfig.video_id)
    parser.add_argument("--num-subactions", type=int, default=E18AConfig.num_subactions)
    parser.add_argument("--max-ref-backtrack", type=int, default=E18AConfig.max_ref_backtrack)
    parser.add_argument("--crop-size", type=int, default=E18AConfig.crop_size)
    parser.add_argument("--output-root", type=Path, default=E18AConfig.output_root)
