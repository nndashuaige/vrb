"""E15 experiment: paper-faithful strict humanless reference."""
...
class E15Config:
    ...
    output_root: Path = VRBREPRODUCTION_ROOT / "outputs" / "e15"
...
EXPERIMENT = {
    "experiment_id": "E15",
    "name": "E15_paper_faithful_reference",
    "contact_strategy": "first_new_contact_episode_first_timestep",
    "reference_strategy": "paper_faithful_strict_humanless_reference",
    "reference_fallback": "none",
    "description": (
        "每个 subaction 只取第一个 smoothed contact 0->1 episode 的 first contact timestep；"
        "reference 只使用 contact episode start 之前最近的 strict global no-hand frame；"
        "不使用 E14 clean hand-object/crop gate，不引入 object consistency、CoTracker、mask tracking 或额外 optical flow。"
    ),
}
...
def get_subactions(config: E15Config) -> pd.DataFrame:
...
class Problem3CachedRunnerE15:
    ...
    def find_paper_faithful_strict_humanless_reference(
        self,
        *,
        t_contact: int,
        active_hand: str,
        ...
        if strict_ref_idx is None:
            detail["reference_reject_reasons"] = {"no_strict_humanless_reference": 1}
            return None, detail

        ref_idx = int(strict_ref_idx)
        ...
        detail.update(
            reference_mode="strict_global_no_hand",
            reference_reason="strict_no_hand_streak_before_episode_start",
    ...
    def run_cell3(
        ...
        *,
        ...
        if search_end < search_start:
            result.update(
                discard=True,
                discard_reason="no_strict_humanless_reference",
                status="DISCARD",
                no_pre_episode_reference_window_not_rescued=True,
                reference_mode="no_strict_humanless_reference",
                reference_reason="no_strict_humanless_reference",
            )
            return result
        ...
        ref_idx, detail = self.find_paper_faithful_strict_humanless_reference(
...
def run_one_candidate(
    *,
    ...
    problem3_runner: Problem3CachedRunnerE15,
    config: E15Config,
    subaction_new_contact_count: int,
    raw_candidate_count: int,
) -> Dict[str, Any]:
    ...
    if cell3.get("discard") or cell3.get("status") != "KEEP":
        ...
        failed_stage = "reference_selection" if reason == "no_strict_humanless_reference" else "Cell3_homography_geometry"
        ...
        [
            {
                ...
                "clean_ref_found": 0,
                "strict_ref_keep": int(((candidate_df["status"] == "keep") & (candidate_df.get("reference_mode") == "strict_global_no_hand")).sum()) if "reference_mode" in candidate_df else 0,
                "fallback_ref_used": 0,
...
def save_bar_chart_failure_reasons(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E15 failure reasons")
...
def save_funnel_chart(overview_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_ylabel("count")
    ax.set_title("E15 pipeline funnel")
...
def save_timeline_chart(candidate_df: pd.DataFrame, subactions: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("Timeline: E15 first-contact samples and reference arrows")
...
def save_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E15 success ref_gap distribution")
...
def save_candidate_offset_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E15 success candidate offset distribution")
...
def save_gap_length_histogram(candidate_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E15 pre-contact gap length distribution")
...
def save_reference_mode_ref_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E15 success ref_gap by reference mode")
    ax.set_xlabel("candidate frame - reference frame")
    ax.set_ylabel("count")
    ax.legend()
    plt.tight_layout()
...
def write_comparison_files(candidate_df: pd.DataFrame, output_root: Path) -> None:
    ...
    for baseline_exp, baseline_root_name, out_name in [
        ("E6b", "数据处理小批量测试前100个subaction_E6b_pre_contact_gap_crop_fallback", "e15_success_vs_e6b.csv"),
        ("E6c", "数据处理小批量测试前100个subaction_E6c_object_consistency", "e15_success_vs_e6c.csv"),
    ]:
    ...
    *,
    ...
    for idx, row in compare_overview.iterrows():
        if not row.get("reference_anchor_gap_median_keep"):
            ...
            baseline_root = resolve_output_dir("e14") if exp_id == "E14" else None
            if exp_id == "E6b":
    ...
    lines = [
        ...
        "## success 分布",
        "",
        f"- strict-global-no-hand ref_gap stats: `{distribution_stats(success_df['ref_gap']) if 'ref_gap' in success_df else distribution_stats(pd.Series(dtype=float))}`",
        ...
        f"- strict reference anchor gap stats: `{distribution_stats(candidate_df['strict_reference_anchor_gap']) if 'strict_reference_anchor_gap' in candidate_df else distribution_stats(pd.Series(dtype=float))}`",
...
def save_reference_anchor_gap_histogram(success_df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("E15 success reference_anchor_gap distribution")
...
def run_e15_experiment(config: Optional[E15Config] = None) -> Dict[str, Any]:
    ...
    for idx, row in subactions.iterrows():
        ...
        print(
            f"E15 subaction {idx:02d} {row['narration_id']} | {row['narration']} | "
    ...
    save_funnel_chart(overview_df, charts_dir / "e15_pipeline_funnel.png")
    save_bar_chart_failure_reasons(candidate_df, charts_dir / "e15_failure_reasons.png")
    save_timeline_chart(candidate_df, subactions, charts_dir / "e15_timeline.png")
    success_df = candidate_df[candidate_df["status"] == "keep"].copy()
    save_ref_gap_histogram(success_df, charts_dir / "e15_ref_gap_success_hist.png")
    save_reference_anchor_gap_histogram(success_df, charts_dir / "e15_reference_anchor_gap_hist.png")
    save_candidate_offset_histogram(success_df, charts_dir / "e15_candidate_offset_success_hist.png")
    save_gap_length_histogram(candidate_df, charts_dir / "e15_gap_length_hist.png")
    save_reference_mode_ref_gap_histogram(success_df, charts_dir / "e15_ref_gap_success_by_reference_mode_hist.png")
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e15_success_contact_sheet",
        "E15 success samples",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e15_success_contact_sheet_page_001.png",
        "E15 success samples",
    )
    save_paginated_success_contact_sheets(
        success_df,
        charts_dir,
        "e15_success_heatmap_trajectory_contact_sheet",
        "E15 success heatmap + trajectory overlays",
        per_page=30,
    )
    ensure_placeholder_png(
        charts_dir / "e15_success_heatmap_trajectory_contact_sheet_page_001.png",
        "E15 success heatmap + trajectory overlays",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run E15 paper-faithful reference baseline.")
    parser.add_argument("--video-id", default=E15Config.video_id)
    parser.add_argument("--num-subactions", type=int, default=E15Config.num_subactions)
    parser.add_argument("--max-ref-backtrack", type=int, default=E15Config.max_ref_backtrack)
    parser.add_argument("--crop-size", type=int, default=E15Config.crop_size)
    parser.add_argument("--output-root", type=Path, default=E15Config.output_root)
